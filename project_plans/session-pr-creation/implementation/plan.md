...
##### Task 1.3.1c: Draft the body via `DraftPRDescription` with fallback (~4 min)
- Compute the diff: `diff, _, diffErr := session.GetGitDiff(ctx,
  wt.WorktreePath(), inst.Worktree.BaseCommitSHA)` (mirror
  `pushAndCreatePR`'s call shape at `backlog_lifecycle.go:3648`; add a
  `WorktreePath()` accessor alongside `RepoPath()` if needed, same caveat as
  1.3.1b).
- If `diffErr != nil` or `strings.TrimSpace(diff) == ""`, skip the LLM call
  and use a deterministic fallback body (a short "## Summary\n\n_No diff
  description available._" placeholder — this session has no backlog item,
  so `buildFallbackPRBody`'s item-description-driven template does not apply
  verbatim; write a minimal session-shaped equivalent).
- Otherwise, call `body, draftErr := headless.DraftPRDescription(ctx,
  s.headlessPool, inst.Title, sessionGoalText(inst), diff, wt.BranchName())`
  ...
  `draftErr != nil`, log a warning and fall back to the same deterministic
  body as the empty-diff case (do not fail the whole RPC — a draft failure
  should never block the modal from opening, matching build-vs-buy.md's
  "fallback template must be first-class" conclusion).
- Files: `server/services/session_service.go`
...
##### Task 1.4.1b: Commit dirty state + push (unconditionally, before existing-PR check) (~4 min)
- Mirror `pushAndCreatePR`'s ordering (`backlog_lifecycle.go:3607-3617`): call
  ...
  (pre-PR)", inst.Title))`, logging (not failing) on error, then call
  `wt.PushBranch()` — on error, return `connect.CodeUnavailable` with the
  push error's exact message (AC6).
- This push must happen *before* any existing-PR short-circuit, per
...
##### Task 1.5.1h: Run and verify the full package (~2 min)
...
- AC7 (single entry point): *Given* `SessionActionsOverflow.tsx`'s overflow
  menu, *When* this story ships, *Then* the menu's PR-related item opens
  `CreatePullRequestModal` (button label cycle "Create PR" /
  "PR pending…"/ derived-from-`session.githubPrUrl` "✅ View PR") and there is
  no code path in this file that calls `onRunOneShot`/`RunOneShot` anymore.
