...
| `session/pipeline_engine.go` (486 lines before this project; 555 lines after) | `ExecutorFor` (Epic 1.5) and `ComputeExecutorHash` (Epic 2.1/Task 5.2.4a) add 69 lines, pushing the file past kibitzer's 500-line advisory threshold for the first time — added retroactively during Phase 6 review after the file crossed the line with no disposition recorded at plan time | **Extend as-is, immaterial** — `ExecutorFor` is the `PipelineEngine` interface's 6th method, following the identical 3-branch fallback shape (`PipelineModeDefault` → defaults; unresolved slug → Warn + defaults; resolved but role absent → defaults) as the other 5 methods — same responsibility extended to a new facet, not a new responsibility bolted on. `ComputeExecutorHash` is a small, pure, heavily-documented free function. No SRP violation; no file split warranted | Phase 6 architecture review (2026-09-17) independently verified the addition is narrowly scoped and low-complexity — recorded here per this table's own stated purpose (a reviewer shouldn't have to reverse-engineer why an oversized file grew), even though the crossing happened during this project rather than pre-existing it like the three rows above |
