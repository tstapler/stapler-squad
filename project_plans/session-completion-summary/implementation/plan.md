...
| `SessionSummaryGenerator` | Domain-level orchestrator owning the `headless.PoolClient`, the ent client, and the FR-7 dedup map. `GenerateAndPersist(ctx context.Context, sessionUUID, sessionTitle string, createdAt time.Time, diffStats *git.DiffStats, sessionGoal *SessionGoalData, reason string)` runs the full async pipeline; its method set satisfies the `summaryGenerator` interface consumed by `sessionSummaryListener`. `sessionGoal` is the session's live `*SessionGoalData` (nil if never set), captured synchronously via `Instance.GetSessionGoal()` at dispatch time — same cheap in-memory-read pattern as `diffStats`, no I/O. | `session/session_summary_service.go` |
...
| `GenerateSessionCompletionNarrative` | `func(ctx context.Context, pool *Pool, sessionTitle, sessionGoal, diff, decisionsSummary string) (text string, err error)` — follows `DraftPRDescription`'s shape (plain string params, no request-struct wrapper, per this file's existing convention of concrete params over a single-call-site struct type). `sessionTitle`/`sessionGoal` are the added grounding inputs (pre-mortem finding #1, `implementation/pre-mortem.md` — the original diff-plus-decisions-only grounding produced generic prose for low-diff/high-effort sessions); `sessionGoal` is `""` when the session never had a goal set. | `session/headless/features.go` |
...
| Narrative input scope | Ground `GenerateSessionCompletionNarrative` in diff + decisions + session title/goal (`Instance.GetSessionGoal()`, sync in-memory read, no I/O) — a modest, cheap widening beyond diff+decisions alone | `implementation/pre-mortem.md` finding #1 (P1); `requirements.md` FR-2 item 1 (wording updated to match) | (1) diff+decisions only (original design); (2) full tool-call-history digest | (1) produces generic, low-value prose for low-diff/high-effort sessions (exploration/investigation work with little or no diff) — the LLM has almost nothing to describe beyond the diff stat, per pre-mortem #1's real product-risk framing. (2) is pre-mortem #1's own prevention text's fallback-if-still-generic option, not the immediate default — ingesting full tool-call transcripts is a materially larger scope increase (transcript retrieval, additional truncation/sanitization, higher token cost) not justified until the cheaper goal/title addition is shown insufficient |
...
- **Never-started early-return path (confirmed correct, not a bug)**: `Destroy()` fires `EventStopped` unconditionally via `defer`, including on an early-return path *before* line 1271's `UpdateDiffStats()`/`CleanupWorktree()` call — e.g. a session that never reached a state where a worktree/diff would exist. In that case `i.GetDiffStats()` returns the zero value (no worktree was ever created, so there is nothing to diff), and the resulting `DiffSnapshot` is empty (`IsEmpty() == true`) — this is the *accurate* representation of "this session never did anything," not a missed-capture bug, and downstream `isTrivialSession`/empty-state rendering already handle an empty `DiffSnapshot` correctly (Epic 1.4/1.5.1). No code change needed for this path; per `implementation/validation.md`'s Requirement → Test Mapping table, this scenario gets its own explicit `session/instance_test.go` case (`Destroy_should_FireEventStoppedWithEmptyDiff_When_InstanceNeverStarted`), a sibling of the existing `Destroy_should_CaptureDiffStatsBeforeCleanupWorktree_When_UpdateDiffStatsRunsFirst` row — same as that row, this test has no dedicated plan.md task of its own (Task 1.1.1a has no dedicated test task, per validation.md's note there), it's covered directly by validation.md's coverage-mapping pass.
...
**Acceptance Criteria**:
- AC-1: *Given* an `Instance` (`UUID: "sess-123"`) whose process exits naturally, *When* `instanceOnExitCallback` fires `i.fireLifecycleEvent(EventExited, "pty-eof")`, *Then* `sessionSummaryListener.OnLifecycleEvent(EventExited, "pty-eof")` calls `instance.GetDiffStats()` and `instance.GetSessionGoal()` synchronously and dispatches `go generator.GenerateAndPersist(context.Background(), "sess-123", <title>, <createdAt>, diffStats, sessionGoal, "pty-eof")` (full signature per Task 1.5.2d; `generator` is the `summaryGenerator` interface field, per Task 1.1.2a) — no manual action required.
...
- `type sessionSummaryListener struct { generator summaryGenerator; instance *Instance }` (note: `generator` is the **interface** type above, not a concrete `*SessionSummaryGenerator` pointer — this is what makes Task 1.1.2b's fake test double constructible without a real ent client). Captures the live `*Instance` pointer, not just its UUID — needed for `GetDiffStats()`/`GetSessionGoal()`/`CreatedAt`/`Title` reads. `OnLifecycleEvent(event LifecycleEvent, reason string)`: `switch event { case EventExited, EventStopped: }` else return; `if reason == "reconcile-session-missing" { return }`; `diffStats := l.instance.GetDiffStats()` (sync, no I/O); `sessionGoal := l.instance.GetSessionGoal()` (sync, no I/O, nil if never set — narrative-grounding input per the Pattern Decisions "Narrative input scope" row); dispatch per Task 1.5.2d's final call shape (`go l.generator.GenerateAndPersist(context.Background(), ...)` — panic recovery for this goroutine lives inside `GenerateAndPersist` itself, see Task 1.5.2b, so it protects this dispatch automatically).
...
**As a** `SessionSummaryGenerator`, **I want** an LLM-generated "what was done" narrative grounded strictly in retrieved data — including the session's own stated goal/title, not diff+decisions alone — **so that** FR-2's narrative section is populated without hallucinating structured facts, and stays informative for low-diff/high-effort sessions (pre-mortem finding #1).
**Acceptance Criteria**:
- AC-2 (partial): *Given* a session title `"fix-login-redirect"`, a session goal string `"Investigate why login redirects loop under SSO"`, a diff (sanitized via `SanitizeDiff`, truncated via the existing `headless.MaxDiffSizeReview` constant), and a `DecisionsSnapshot` rendered to a summary string, *When* `GenerateSessionCompletionNarrative(ctx, pool, sessionTitle, sessionGoal, diff, decisionsSummary)` is called, *Then* it returns prose text and `pool.CallBlocking` was invoked with `FeatureKeySessionCompletionSummary`.
- AC-2 (partial, empty goal): *Given* `sessionGoal == ""` (session never had a goal set), *When* the same function is called, *Then* it still succeeds — the goal line is simply omitted from the user prompt, not rendered as an empty/placeholder line.
**Files**: `session/headless/features.go`

##### Task 1.4.1a: Add `FeatureKeySessionCompletionSummary` + narrative function (~5 min)
- In `session/headless/features.go`, add `FeatureKeySessionCompletionSummary FeatureKey = "session-completion-summary"` to the const block. Add `sessionCompletionSummarySystemPrompt` const (stable prompt instructing: ground strictly in the provided title/goal/diff/decisions/timeline, do not speculate beyond what's given, plain descriptive prose only, no markdown headings — headings are added by the deterministic renderer). Add `func GenerateSessionCompletionNarrative(ctx context.Context, pool *Pool, sessionTitle, sessionGoal, diff, decisionsSummary string) (string, error)`: sanitize+truncate `diff` via the existing `SanitizeDiff`/`maxDiffSizeReview`-equivalent helpers used by `session/backlog_review.go:611-615,646-680` (reuse, don't reinvent per `research/features.md` §1); build the user prompt from title + (if non-empty) goal + diff + decisions — `sessionGoal` is the cheap, modest grounding-input widening from pre-mortem finding #1 (`implementation/pre-mortem.md`), giving the model real signal for low-diff/high-effort sessions (exploration/investigation work with little or no diff) without ingesting a full tool-call-history digest (that remains the documented fallback if goal/title grounding proves insufficient, not the v1 default); call `pool.CallBlocking(ctx, FeatureKeySessionCompletionSummary, sessionCompletionSummarySystemPrompt, userPrompt, CallOptions{})`, return text or error (caller handles fallback).
- Files: `session/headless/features.go`
...
- New `session/session_summary_service.go`: `type SessionSummaryGenerator struct { entClient *ent.Client; pool headless.PoolClient; notifStore *notifications.NotificationHistoryStore; tokenStore *tokens.TokenStore; reviewLookup ReviewQueueLookup; inFlight sync.Map /* session_id -> *sync.Mutex */ }`. `func NewSessionSummaryGenerator(entClient *ent.Client, pool headless.PoolClient, notifStore *notifications.NotificationHistoryStore, tokenStore *tokens.TokenStore, reviewLookup ReviewQueueLookup) *SessionSummaryGenerator`. Its `GenerateAndPersist` method (Tasks 1.5.2b + 1.5.2c together implement this one method's body) is the sole method required to satisfy the `summaryGenerator` interface consumed by `session_summary_listener.go` (Task 1.1.2a) — no explicit interface assertion needed, Go structural typing handles it.
- Files: `session/session_summary_service.go`

##### Task 1.5.2b: `GenerateAndPersist` — guard, panic safety, pre-checks, interim upsert (~5 min)
- Signature (defined here; the body continues in Task 1.5.2c): `func (g *SessionSummaryGenerator) GenerateAndPersist(ctx context.Context, sessionUUID, sessionTitle string, createdAt time.Time, diffStats *git.DiffStats, sessionGoal *SessionGoalData, reason string)`. `sessionGoal` is threaded through to the narrative step (Task 1.5.2c step 4) as the `sessionGoal` string input to `GenerateSessionCompletionNarrative` (`sessionGoal.Goal` if non-nil, else `""`) — see the Pattern Decisions "Narrative input scope" row.
- **Panic safety (mirrors `BacklogLifecycleListener.runStuckDetector`, `session/backlog_lifecycle.go:1407-1421`)**: since this method is always invoked via `go ...GenerateAndPersist(...)` (both the listener, Task 1.5.2d, and Regenerate, Task 2.2.2a, dispatch it as a detached goroutine), an unrecovered panic anywhere in the pipeline (LLM client, markdown rendering, ent write) would crash the entire server process, taking every live tmux session down with it — not just this feature. As the very first two statements of the method: acquire the dedup guard, then `defer release()` immediately, then `defer func() { if r := recover(); r != nil { log.WarningLog.Printf("[SessionSummary] GenerateAndPersist panicked (recovered) for session=%s: %v", sessionUUID, r) } }()`. Because Go runs deferred functions in LIFO order, the recover-wrapper (registered second) runs first and stops the panic, after which `release()` (registered first) still runs normally — so the guard is never left permanently locked by a panic, and the crash never reaches the rest of the process. (A future best-effort improvement — not required for this task — would be for the recover handler to also attempt an ERROR upsert; out of scope here since the panic could have occurred inside the DB call itself.)
- Steps implemented by this task — everything before real (LLM/persist) work starts: (1) `release, ok := g.tryAcquire(sessionUUID)`; if `!ok`, return immediately without touching the DB. `defer release()` right after a successful acquire (not a plain final step — see panic-safety note above). (1b) **Sequential-duplicate short-circuit, keyed on whether this generation's data is actually new (closes pre-mortem finding #5's resumed-session blind spot — `implementation/pre-mortem.md` P1 #5)**: unconditionally compute `incomingDiff := BuildDiffSnapshot(diffStats)` first (pure, no I/O — cheap regardless of which branch below is taken; Task 1.5.2c's step 3 reuses this same value, no recomputation needed there), then read the current persisted row for `sessionUUID` (no row yet — the common first-ever-trigger case — skips straight past this whole check to step 2). If `reason != "manual-regenerate"` and the row's `status == "ready"`: compare `incomingDiff.FilesChanged`/`.Added`/`.Removed` against the persisted row's `diff_files_changed`/`diff_added`/`diff_removed` fields. If all three match exactly, this is a true duplicate/late-arriving trigger (e.g. a duplicate `EventExited`/`EventStopped` for the same completion) — return immediately with no LLM call and no write. If any differ, this is genuinely new data — most commonly a resumed session (`resume_session`) that did substantial new work before exiting a second time — so proceed with regeneration despite the row already being `READY` (do not short-circuit). Known, accepted limitation (not engineered around, per this repo's pragmatic/"ponytail" convention): if the diff content changed while the three counts happen to coincide exactly (rare), this still short-circuits as a false duplicate; a full content-hash comparison would close that residual gap but isn't justified for v1 — the counts comparison is the chosen fix, not a content-diff (pre-mortem #5's own "alternatively... explicit reset" framing is the noted-but-not-chosen fallback). An explicit user-initiated Regenerate (`reason == "manual-regenerate"`, set by Task 2.2.2a) is exempt from this check entirely (no diff-count comparison) and always proceeds. (1c) **Cooldown check for Regenerate** (independent of the in-flight guard, addresses rapid sequential Regenerate clicks — see Task 1.5.3c for the constant): if `reason == "manual-regenerate"` and the row's `generated_at` is non-nil and less than `regenerateCooldown` old, return immediately without a write (current row is unchanged — the caller/RPC handler returns the row as-is). (2) upsert row to `status: "generating"`, `generation_started_at: now` — same `g.entClient.SessionSummary.Create().Set...().OnConflictColumns(sessionsummary.FieldSessionID).UpdateNewValues().Exec(ctx)` shape as Task 1.5.2c's step 6 (not a bare `.Create()`, which would hit `UNIQUE constraint failed: session_id` on any re-trigger for a session that already has a row — the `session_id` field's `Unique()` constraint from Task 1.2.1a makes this the core path, not an edge case). `incomingDiff` (computed in step 1b) is carried forward for Task 1.5.2c's step 3 to reuse — no recomputation.
- Files: `session/session_summary_service.go`

##### Task 1.5.2c: `GenerateAndPersist` — snapshot build, narrative, markdown render, final persist (~5 min)
- Continues the same method body started in Task 1.5.2b (same file, same function — this task picks up immediately after Task 1.5.2b's step 2 interim upsert). Steps: (3) build `TimelineSnapshot`/`DecisionsSnapshot`/`CostSnapshot` via Epic 1.3 functions (`DiffSnapshot` reuses `incomingDiff` already computed in Task 1.5.2b's step 1b — no need to recompute) — on any error from `BuildDecisionsSnapshot(ctx, ...)` (the only builder that returns an error), upsert `status: "error"`, `error_stage: "decisions"`, `error_message`, **and still persist the already-computed `DiffSnapshot`/`TimelineSnapshot` fields** (built earlier in this same step, before the failing call) so a subsequent Regenerate always has *something* — a real diff/timeline, not nothing — to fall back to if the live `*Instance` is also gone by then (see Task 2.2.2a); leave `markdown`/`narrative`/decisions fields from any prior successful generation untouched (field-scoped upsert — see Pattern Decisions "Error-path field preservation"); return (guard release handled by the `defer` registered in Task 1.5.2b); (4) if `isTrivialSession(...)`, set `narrative = narrativeFallbackTrivial, fallbackUsed = true`, skip LLM; else call `GenerateSessionCompletionNarrative(ctx, pool, sessionTitle, goalText, diff, decisionsSummary)` (where `goalText = sessionGoal.Goal` if `sessionGoal != nil` else `""`) under a `context.WithTimeout(ctx, llmNarrativeTimeout)` — on error, `narrative = narrativeFallbackLLMFailure, fallbackUsed = true` (log at Warn, do not abort); (5) render markdown via `RenderSessionSummaryMarkdown`; (6) single final upsert write: all fields + `status: "ready"`, `generated_at: now`, via `g.entClient.SessionSummary.Create().Set...().OnConflictColumns(sessionsummary.FieldSessionID).UpdateNewValues().Exec(ctx)` (mirror the exact upsert shape used in `session/ent_repository_backlog.go:1071-1082` / `server/services/error_registry.go:52`'s `OnConflictColumns(...).Update(...)` — this repo's ent setup requires `--feature sql/upsert`, already covered by Task 1.2.1b). **On this write's own failure** (LLM cost already spent, row still says `"generating"`): attempt a best-effort fallback write, `status: "error", error_stage: "persist"`, and log at Error level — do not silently leave the row in `"generating"` for the 5-minute staleness reconciler to eventually paper over with a misleading "possibly due to a server restart" message.
- Files: `session/session_summary_service.go`

##### Task 1.5.2d: Wire `sessionSummaryListener` to call this with instance-derived args (~2 min)
- Update Task 1.1.2a's `sessionSummaryListener.OnLifecycleEvent` dispatch to `go l.generator.GenerateAndPersist(context.Background(), l.instance.UUID, l.instance.Title, l.instance.CreatedAt, diffStats, sessionGoal, reason)`.
- Files: `session/session_summary_listener.go`

##### Task 1.5.2e: Unit tests for `GenerateAndPersist` — guard/pre-check/cooldown/panic scenarios (~4 min)
- New `session/session_summary_service_test.go` (uses ent's in-memory SQLite test client, matching existing ent test setup convention elsewhere in this package) — covers the Task 1.5.2b portion of the method (guard acquisition, panic/defer safety, sequential-duplicate short-circuit, cooldown check, interim upsert): **sequential-duplicate short-circuit (true duplicate)**: a second call with a non-manual `reason` and an unchanged `diffStats` against an already-`READY` row makes no LLM call and writes nothing; **sequential-duplicate short-circuit bypassed for genuinely new data (pre-mortem P1 #5)**: a resume-then-reexit scenario — a second call with a non-manual `reason` (e.g. `"pty-eof"`) against an already-`READY` row, but with `diffStats` reporting a strictly larger `Added`/`Removed`/`FilesChanged` than the persisted row — proceeds with a full regeneration (LLM called, row re-upserted with the new diff/narrative) despite the row already being `READY`; **manual-regenerate bypass**: a second call with `reason: "manual-regenerate"` against an already-`READY` row (outside the cooldown window) does proceed and re-runs the pipeline regardless of whether the diff changed; **cooldown**: a `manual-regenerate` call within `regenerateCooldown` of the row's `generated_at` returns immediately with no write; **panic recovery**: a fake pool/builder that panics does not crash the test process and leaves the guard released (a follow-up call for the same session succeeds).
- Files: `session/session_summary_service_test.go`

##### Task 1.5.2f: Unit tests for `GenerateAndPersist` — build/narrative/persist/fallback scenarios (~4 min)
- Extends the same `session/session_summary_service_test.go` from Task 1.5.2e (uses ent's in-memory SQLite test client, matching existing ent test setup convention elsewhere in this package) — covers the Task 1.5.2c portion of the method (snapshot build, narrative generation, markdown render, final persist): trivial session → READY with fallback narrative, no LLM call recorded on a fake `headless.PoolClient`; decisions-builder error → ERROR with `error_stage: "decisions"` **and asserts the already-computed diff/timeline fields are persisted on that error row, not discarded**; LLM failure (fake pool returns error) → READY with `narrative_fallback_used: true`; happy path → READY with all fields populated and exactly one *final* write (assert via row's final state, and that no intermediate GENERATING row is left dangling); **persist-write failure**: a fake ent client that fails only on the final upsert results in `status: "error", error_stage: "persist"`.
- Files: `session/session_summary_service_test.go`
...
- AC-8 (sequential-duplicate case, true duplicate): *Given* a row for `"sess-123"` already `status: "ready"` (guard free, generation finished) with persisted `diff_added: 42, diff_removed: 7, diff_files_changed: 3`, *When* a late-arriving duplicate `EventExited`/`EventStopped` calls `GenerateAndPersist("sess-123", ..., diffStats: {Added: 42, Removed: 7} /* same 3 files */, reason: "pty-eof")` again, *Then* it returns immediately with no LLM call and no write (Task 1.5.2b step 1b) — but an explicit `RegenerateSessionSummary` call (`reason: "manual-regenerate"`) against the same `READY` row still proceeds.
- AC-8 (sequential-duplicate case, genuinely-new data — closes pre-mortem P1 #5): *Given* the same `"sess-123"` row `status: "ready"` with persisted `diff_added: 42, diff_removed: 7, diff_files_changed: 3` (e.g. the session was paused via `pause_session`, resumed via `resume_session`, and did substantial new work), *When* the resumed session exits again and `GenerateAndPersist("sess-123", ..., diffStats: {Added: 90, Removed: 12} /* 5 files */, reason: "pty-eof")` is called, *Then* the diff-count comparison in Task 1.5.2b step 1b detects the mismatch, does **not** short-circuit, and proceeds with a full regeneration — the row is re-upserted reflecting the new, larger diff and a fresh narrative, not left pinned to the pre-resume state.
...
- **Note on requirements.md NFR wording**: `requirements.md`'s NFR text ("the narrative LLM call happens at most once per successful generation per session") describes explicit Regenerate as an intentional exception to the "at most once" restriction on automatic triggers — this 30s cooldown does not contradict that: it is a research-derived (`research/pitfalls.md` §7) anti-abuse safeguard against *rapid repeated* clicks specifically, not a general throttle on Regenerate's per-generation LLM-call intent. A single Regenerate click after the cooldown window has elapsed always proceeds and always makes exactly one LLM call, same as the NFR describes.
- Files: `session/session_summary_service.go`
...
##### Task 2.2.2a: `RegenerateSessionSummary` handler (~7 min — widened from an original ~4 min estimate; live/no-live instance branching, sessionGoal handling, and the detached-context dispatch reasoning below are each individually simple but genuinely add up to more than a 4-minute task. No clean task-boundary seam exists between "resolve which diff/timeline/goal source to use" and "dispatch as a detached goroutine" — both bullets below write into the same handler function and the second bullet's reasoning only makes sense once the first bullet's inputs are already assembled — so this stays one task with a wider estimate rather than being forced apart, per the coordinator's discretion on this lower-severity finding.)
- In `server/services/session_summary_service.go`: look up the row for context (`session_title`, need `Instance.CreatedAt`/live diff — if the live `*Instance` is still available via `SessionService.FindLiveInstance`, refresh the diff snapshot **and the session goal** (`instance.GetSessionGoal()`) the same way the listener does; if not (session long gone), reuse the previously-persisted diff/timeline/cost fields as the input, pass `sessionGoal: nil` (no live instance to read it from — the persisted row doesn't store the goal, only its effect on a prior narrative), and only re-run the narrative + decisions steps). **"Live instance gone + row previously errored before diff/timeline were ever persisted"**: this gap is closed by Task 1.5.2c's error-path fix — a decisions-stage failure now always persists the already-computed `DiffSnapshot`/`TimelineSnapshot` fields even though the row ends in ERROR, so this fallback always has *something* real to reuse (not nothing) for any row that ever reached step 3 of `GenerateAndPersist`, regardless of which stage failed.
- **Critical: dispatch as a detached goroutine with a background context, not the request-scoped `ctx`.** Call `go g.GenerateAndPersist(context.Background(), sessionID, title, createdAt, diffStats, sessionGoal, "manual-regenerate")` — **exactly the same shape as Task 1.5.2d's listener dispatch**, never `g.GenerateAndPersist(ctx, ...)` (without `go`) or `go g.GenerateAndPersist(ctx, ...)` (with the request's `ctx`). ConnectRPC/`net/http` cancels the request-scoped `ctx` as soon as the handler returns; since this handler is required to "return the current row state immediately (don't block on the async goroutine — client polls)," the pipeline necessarily keeps running after the handler returns, so it must not be tied to a context that dies the moment the handler returns — otherwise the narrative LLM call's own `context.WithTimeout(ctx, llmNarrativeTimeout)` (Task 1.5.2c step 4) would be cancelled almost instantly, silently degrading Regenerate to the LLM-failure fallback text on essentially every invocation. The dedup guard (Task 1.5.3a) and the cooldown/sequential-duplicate short-circuits (Task 1.5.2b steps 1b/1c) handle overlap and rate-limiting; the RPC handler itself does not need to wait for or synchronize with the goroutine. Return the current row state immediately.
- Files: `server/services/session_summary_service.go`

##### Task 2.2.2b: Unit tests for both RPC handlers (~7 min)
- New `server/services/session_summary_service_test.go`: `GetSessionSummary` returns `Summary: nil` for a session with no row; returns the row when `Session` doesn't exist in storage (assert no call to `s.storage`); staleness reconciliation flips an old `GENERATING` row to `ERROR` on read **only when the in-memory guard is not held** (add a case with the guard held via a fake `isInFlight` returning `true` — the row stays unchanged). `RegenerateSessionSummary` on an already-`GENERATING` row does not trigger a second `pool.CallBlocking` (assert via fake pool's call count); **Regenerate with a live instance still present** — diff snapshot and session goal are both refreshed the same way the listener does; **Regenerate with no live instance** (`SessionService.FindLiveInstance` returns not-found) — falls back to the previously-persisted diff/timeline/cost fields as input, passes `sessionGoal: nil`, including the case where those fields came from a prior *error* row (Task 1.5.2c's error-path persistence fix) rather than a prior READY row; **Regenerate dispatches with `context.Background()`, not the request `ctx`** — assert the goroutine's context is not cancelled when the handler's own request context is cancelled immediately after the RPC returns (e.g. pass a `ctx` that's already cancelled by the time `GenerateAndPersist` runs, and assert the narrative call still completes rather than immediately erroring with `context.Canceled`).
...
- New `web-app/src/components/sessions/SessionSummaryPanel.tsx`: `export function SessionSummaryPanel({ sessionId }: { sessionId: string })`, uses `useSessionSummary(sessionId)`. Renders: PENDING/GENERATING (or `data === null` and not yet `neverResolved`) → the exact 17-block skeleton layout (5 section placeholders, `data-testid="summary-skeleton-block"` per block, styled via Task 3.1.2b's `skeletonBlock`/reduced-motion styles — see Task 3.1.2b for the precise block-count/shape table from `design/ux.md` surface (b)) + "Generating summary…" (`aria-busy="true"` on the panel container, `aria-live="polite"` status text, per `research/ux.md` §3); `neverResolved: true` → terminal "No summary available for this session" empty state (no further polling — see Task 3.1.1a); READY → `<ReactMarkdown remarkPlugins={[remarkGfm]}>{data.markdown}</ReactMarkdown>` (reusing the existing `react-markdown`+`remark-gfm` pattern from `DescriptionSection.tsx`) plus the Decisions breakdown rendered prominently near the top (not buried — per `research/ux.md` §5); **ERROR with `data.markdown` empty** → bare error state + Regenerate button per the AC above; **ERROR with `data.markdown` non-empty** → the READY rendering plus the stale-document banner described in the AC above (branch on `status === "ERROR" && !!data.markdown`, not on `status` alone); no row yet (session still running) → disabled-tab state is handled by the tab wrapper (Story 3.2.1), this component assumes it's only rendered once reachable.
- Files: `web-app/src/components/sessions/SessionSummaryPanel.tsx`

##### Task 3.1.2b: `.css.ts` styling (vanilla-extract) (~4 min)
- New `web-app/src/components/sessions/SessionSummaryPanel.css.ts` per `.claude/rules/css-architecture.md` (new component → vanilla-extract, not a `.module.css`): tokens from `web-app/src/styles/theme.css.ts` for spacing/colors; no hardcoded hex/`var()` strings.
- **Skeleton spec — exact layout from `design/ux.md` surface (b), not a vague placeholder**: the GENERATING state renders **17 total skeleton blocks** across 5 section placeholders (Decisions: 1 heading bar + 5 pill blocks; What Was Done: 1 heading bar + 3 text-line bars (last at 60% width); Changes: 1 heading bar + 1 stat-line bar + 1 link-line bar (40% width); Timeline: 1 heading bar + 1 text-line bar; Token Usage: 1 heading bar + 1 text-line bar — 5 heading bars + 12 content bars = 17), each block carrying `data-testid="summary-skeleton-block"` (rendered by the component in Task 3.1.2a; this file defines the `skeletonBlock` style those elements use, shaped/sized per the table in `design/ux.md` surface (b)). Add a `skeletonBlockReducedMotion` (or a `prefers-reduced-motion` media-query variant on `skeletonBlock` itself) implementing UX-AC-14: the shimmer animation (`background-position` keyframe) applies only outside `@media (prefers-reduced-motion: reduce)`; inside that query the block renders as a static gray block with no animation.
- Files: `web-app/src/components/sessions/SessionSummaryPanel.css.ts`

##### Task 3.1.2c: Component tests (~7 min)
- New `web-app/src/components/sessions/SessionSummaryPanel.test.tsx` (Jest + RTL, mocked `useSessionSummary`): renders skeleton for GENERATING, asserting exactly 17 elements with `data-testid="summary-skeleton-block"` (UX-AC per `design/ux.md` surface (b)'s exact spec); **renders skeleton blocks without the shimmer animation class/style when `prefers-reduced-motion: reduce` is mocked via `window.matchMedia`** (UX-AC-14, closes the previously-unimplemented gap); renders markdown + decisions for READY; renders bare error + enabled Regenerate for ERROR with empty `markdown`; **renders the READY document with a stale-document banner (not the bare error card) for ERROR with non-empty `markdown`**, and asserts the banner's "Try again" action, not a duplicate toolbar Regenerate button; **renders the terminal "No summary available" empty state when `neverResolved: true`**; Regenerate button becomes disabled with "Regenerating…" label while `regenerate()` is in flight; Copy button triggers `copy()` and announces success/failure via the `aria-live` region (assert the region's text content, not just that `copyToClipboard` was called).
...
**Note on requirements.md grounding**: `requirements.md`'s FR-3 grounding text names "session history/search" as an illustrative example of a stable-identifier retrieval path; `NotificationsPage.tsx` is a plan-phase design choice, not a literal citation of that example — it was verified as the one existing surface that already renders session-scoped links today, whereas a session history/search surface does not currently exist in this codebase and would be a materially larger, separately-scoped feature to build just for this discoverability requirement.
**Acceptance Criteria**:
