...
- `type sessionSummaryListener struct { generator summaryGenerator; instance *Instance }` (note: `generator` is the **interface** type above, not a concrete `*SessionSummaryGenerator` pointer — this is what makes Task 1.1.2b's fake test double constructible without a real ent client). Captures the live `*Instance` pointer, not just its UUID — needed for `GetDiffStats()`/`GetSessionGoal()`/`CreatedAt`/`Title` reads. `OnLifecycleEvent(event LifecycleEvent, reason string)`: `switch event { case EventExited, EventStopped: }` else return; `if reason == "reconcile-session-missing" { return }`; `diffStats := l.instance.GetDiffStats()` (sync, no I/O); `sessionGoal := l.instance.GetSessionGoal()` (sync, no I/O, nil if never set — narrative-grounding input per the Pattern Decisions "Narrative input scope" row); dispatch per Task 1.5.2c's final call shape (`go l.generator.GenerateAndPersist(context.Background(), ...)` — panic recovery for this goroutine lives inside `GenerateAndPersist` itself, see Task 1.5.2b, so it protects this dispatch automatically).
...
- Signature: `func (g *SessionSummaryGenerator) GenerateAndPersist(ctx context.Context, sessionUUID, sessionTitle string, createdAt time.Time, diffStats *git.DiffStats, sessionGoal *SessionGoalData, reason string)`. `sessionGoal` is threaded through to the narrative step (step 4) as the `sessionGoal` string input to `GenerateSessionCompletionNarrative` (`sessionGoal.Goal` if non-nil, else `""`) — see the Pattern Decisions "Narrative input scope" row.
