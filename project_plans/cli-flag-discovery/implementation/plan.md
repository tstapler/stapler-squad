...
**Date**: 2026-09-21 (repaired after adversarial + architecture review, phase 4 and the triad review; see the repair logs at the end)
**Status**: Ready for implementation of Milestone M1 (walking skeleton); three human decisions are PENDING (block below); implementation proceeds on coordinator decision. Nothing below records a human approval.
...
## Pending human decisions (all PENDING; none is approved)

These three need Tyler. Each is a product or security-model call, not an engineering one. Implementation of M1 proceeds on the coordinator's decision; if any is rejected, the fallback named beside it applies.

| # | Decision | Where recorded | If rejected |
|---|---|---|---|
| H1 | **ADR-001 sign-off**: probe any single UI-supplied command with hardened execution plus the request guard (reads AC8 as one command per request, not a saved-program allow-list). | ADR-001; Flagged Choice 1; `requirements.md` Interpretation notes | Escalate to the additive option B (saved-only allow-list); Program Config's unsaved-form check (AC5) then degrades |
| H2 | **AC7 scope-down**: session creation validates the selected program's saved `cli_flags` only; alias `extraFlags` are not validated. | Flagged Choice 2; Unresolved Questions | Plumb `extraFlags` into the panel (touches two hotspot files) |
| H3 | **First-click cost for shebang CLIs**: claude, gemini and aider are scripts, so each needs one explicit Check before flags appear; blur and picker selection never run them. | Flagged Choice 12; Unresolved Questions | Allow-list of known CLI basenames that skip confirmation (a security-model change, goes through ADR-001) |

---

## Delivery order, MVP cut line and effort

**Order of work (changed after the triad review).** (1) Task 1.0.1, the timing and side-effect measurement, is the **first task**: a `PENDING`/`slowTools` outcome changes `ProbeStatus`, so it must update the proto (Task 1.2.1a) **before** `make proto-gen`, not after. (2) **Milestone M1, walking skeleton**: resolve plus found/not-found RPC (no `--help` execution) with the badge in both UIs. (3) **Gate G1**: parse the captured fixtures. (4) **Milestone M2**: `--help` execution, parser, autocomplete, warnings, descriptions.

**MVP cut line = end of M1.** M1 delivers AC2, AC5 (found/not-found badge) and the AC7 badge for saved programs. In M1 the `Prober` is built without a `RunFunc`, so `Probe` stops after resolve and stat and returns `FOUND_NO_FLAGS` (path returned); the badge shows "Found: <path>" and the "Couldn't read flags from --help" sentence is added only when execution lands (Task 2.1.2c). Everything else (AC1 flags, AC3, AC4, AC6, AC9) is M2.

**Kill criterion (Gate G1).** After M1, capture the fixtures (Task 1.1.3a) and run `ParseHelp` (Tasks 1.1.3b-c) over them for the five value-risk targets (claude, aider, gemini, agy, gh). Continue into M2 only if **at least 3 of the 5 produce more than 0 parsed flags** and Task 1.0.1 shows none needs a bound that forces `PENDING`. Otherwise stop after M1: keep the badge, drop autocomplete, warnings and descriptions (Phases 3 and 4), and record the outcome in the repair log. This is the cheap check on the value-risk assumption in `requirements.md` ("help output of the target CLIs parses into usable flags").

**Gate G1 result (2026-09-21): PASS, 5 of 5.** `ParseHelp` (`config/clihelp/parser.go`) over the committed fixtures parsed claude 65 flags, aider 124, gemini 29, agy 24, gh 2 (top-level help lists only `--help`/`--version`; `gh pr list` parses 16), against the threshold of 3 of 5 with more than 0 flags (`go test ./config/clihelp -run GateG1 -v`). The second condition (no target forces `PENDING`) was already met by Task 1.0.1. Spot-check of 3+ flags per fixture against the raw text found names, shorts, aliases and descriptions correct (for example claude `--allowedTools` with alias `--allowed-tools`, aider `--model` with `TakesValue`, gemini `-d/--debug`, agy `-c`). Systematic limits, none blocking: agy and gemini show no value hint for some flags, so `TakesValue` is unreliable there (gemini is recovered from its `[string]`/`[array]` tags, agy is not); agy's `-c` and `--continue` stay two entries because the help never links them; `-., --hidden` in rg is dropped because `-.` fails the name-shape check; descriptions keep yargs `[boolean] [default: ...]` tags. Continue into M2.

**Effort (hours, per task, 1-4h; recounted from the task headings below by script).** The previous "3-6 min" labels were not realistic. Hour figures are estimates for one implementer including tests, not measurements.

| Segment | Tasks | Hours |
|---|---|---|
| Phase 1, M1 part (1.0.1, resolve, Probe part 1, login PATH, proto, handler, guard, kill switch) | 20 | 40.5 |
| Phase 2 (badge in both UIs, hook, ProgramsManager, picker; part of M1, except Task 2.1.2c which is M2) | 11 | 28 |
| Phase 1, M2 part (runner, parser, cache, gate, Probe part 2; behind Gate G1) | 14 | 36.5 |
| Phase 3 (autocomplete) | 6 | 16.5 |
| Phase 4 (warnings, descriptions, AC9 e2e) | 9 | 21 |
| Phase 5 (desktop e2e, gates, screen-reader checklist) | 3 | 8.5 |
| **Total** | **63** | **151** |

M1 (Phase 1 M1 part + Phase 2) is about 68.5h before Gate G1 (Task 2.1.2c's 1h counts in Phase 2 but ships with M2). Recount: `grep -E '^##### Task' implementation/plan.md` and sum the `(Nh)` labels.
...
8. **Live-settable kill switch, default ON.** Repo rule: rollout flags are live-settable through the feature-flag RPC and Settings panel, never env vars. The mechanism exists and is light: an entry in `knownFeatureFlags` (`server/services/feature_flag_service.go`, with `defaultValue: true`), read per request with `config.LoadConfig().GetFeatureFlagWithDefault(name, true)`, enforced by `interceptors.NewScopedFeatureFlagInterceptor` (`server/interceptors/feature_flag_interceptor.go`, registered as at `server/server.go:678`), toggled from `web-app/src/app/settings/features/page.tsx` (`FEATURE_META`). Flag `programs:cli-flag-probe` gates the `ProbeProgram` RPC; when off it returns `CodeUnimplemented` and the UI hides the badge and combobox (Tasks 1.2.4a-b). Justification: the RPC executes user-named programs, and ADR-001 sign-off is PENDING, so the owner needs an off switch that does not need a deploy.
9. **UX decisions from `design/ux.md`**: D1 adopted **with a change** (Enter in the command field never submits the form: for a non-script program it runs Check; for a script it does **not** run it and shows the NEEDS_CONFIRM hint, so consent to execute is only ever the explicit Check button; task 2.2.1a). D2 adopted (copy "Checked on this server only" shown in the badge detail for every probe, since the probe only ever sees the server host; task 2.1.2a). D3 adopted (inline description on the active option, no button inside `role=option`; tasks 4.2.2a). D4 **dropped**: no "Did you mean / Use it" and no "Edit in Program Config" link. D5 adopted (Check button on desktop and mobile). Also dropped as scope drift: `docs/reference/program-probe.md` and its `CLAUDE.md` index row. Kept: `docs/registry/features/` regeneration (repo rule) and the e2e spec (AC10 requires it; AC9 proof).
...
11. **Timeout is a measured default, not a constant (pre-mortem P1-1).** AC3's 3s stays the default. Task 1.0.1 (the first task of the plan) captures cold and warm `--help` wall time for claude, aider, gemini, agy and gh and records the numbers in the "Measured `--help` timings" table below. Decision rule: if **any** target exceeds about 1.5s cold, `Limits.Timeout` becomes overridable per probe (a `slowTools` map keyed by basename in `config/clihelp`, raised to a value derived from the measurement, e.g. 2x the worst cold time capped at 10s) while the default stays 3s; if a target still cannot finish inside a sensible bound, add a `PENDING` status (RPC returns fast, flight continues under `WithoutCancel`, client re-polls) instead of a bigger timeout. Whichever branch is taken is recorded in that table and in the repair log; a `slowTools` change touches only `config/clihelp`, but a `PENDING` status adds an enum value and so must be applied to `session.proto` (Task 1.2.1a) **before** `make proto-gen` and before any UI code is written, which is why the measurement is the first task. `TIMEOUT` is shown as "Found: /path. Timed out reading flags — try Check again." (never as "no flags"), cached 60s (was 30s; still short so a busy-machine timeout heals), and an explicit Check bypasses a cached `TIMEOUT`. A real-binary smoke test (skipped when the binary is absent) guards the decision.
12. **Scripts are not run without an explicit Check (pre-mortem P1-2; ADR-001 addition, sign-off PENDING).** `Prober` reads the first bytes of the resolved file: native binary (ELF/Mach-O) may run on an implicit probe; a `#!` shebang or unrecognized file returns `NEEDS_CONFIRM` without executing. Execution needs request field `confirm_execute=true`, sent only by the Check button (never by blur, never by Enter, never by selection). The session-creation picker never executes an **unconfirmed** target on selection: it sends `resolve_only=true` (found-only, cached result if any, else `NEEDS_CONFIRM`) and offers a `Check` button. Confirmed (real path, mtime, size) targets are remembered for the process lifetime, and **`resolve_only` honours that set**: a confirmed target is probed (or served from cache) on selection, so a program the user already checked never regresses to `NEEDS_CONFIRM` after the 10-minute cache TTL expires. Two cases still show `NEEDS_CONFIRM` again, on purpose: a changed mtime or size (different bytes, the earlier consent no longer covers them) and a server restart (the confirmed set is in memory; persisting execution consent to disk is a security-model change, deferred and recorded in Unresolved Questions). Cost: claude, gemini and aider are shebang scripts, so each needs one click the first time per server process (and after a tool upgrade); recorded in Unresolved Questions and Pending decision H3.

### Measured `--help` timings (fill during Task 1.0.1, the first task; go/no-go)
...
| `Limits` | `{Timeout time.Duration; MaxBytes int}`; defaults 3s / 256KiB; per-tool override via `slowTools` if the Task 1.0.1 measurement requires (Flagged Choice 11) | Passed to `RunFunc` on every call so tests can assert plumbing |
...
## Risk Control
- **Feature flag**: `programs:cli-flag-probe`, live-settable via the existing feature-flag RPC and Settings panel, default ON (Flagged Choice 8; Tasks 1.2.4a-b). Off means the RPC returns `Unimplemented` and the UI shows the pre-feature plain inputs. No env var.
- **Rollback procedure**: turn the flag off (no deploy), or revert the PR; proto change is additive.
...
- [ ] `git --help` under a pipe may spawn `man`; with the sanitized env (`PAGER=cat`, `MANPAGER=cat`) verify and, if it does, use `git -h` output as the negative fixture — observed in task 1.0.1, consumed by 1.1.3a; owner: implementer.
...
- [ ] Does `--help` for claude/aider/gemini write to `$HOME` (config, update checks)? UNVERIFIED (adversarial review). Implementer records the result in task 1.0.1 (same runs as the timing measurement). If it does, point `HOME` at the temp dir for the child (lookup still uses the real home).
- [x] **RESOLVED (Task 1.2.3p, 2026-09-21, manual instance `claude-manual-test` on `PORT=62871`):** accessor is the existing lock-free `(*Server).GetAddr()`; the guard tests `middleware.ListenAddrIsLoopback(s.GetAddr())` per request (`net.SplitHostPort`, `localhost`/loopback IP literals count, `:port` and `0.0.0.0` do not). Deployed default: configured `localhost:8543` (`main.go` address resolution: flag > `PORT` env > config > default), rewritten after bind to `127.0.0.1:<port>` (`dualStackListener.Addr()` is the IPv4 side); both classify as loopback. Observed against the manual instance: `Host` of `localhost:62871`, `127.0.0.1:62871` and `[::1]:62871` returned 200, `evil.example:62871` 403, GET 405, a foreign `Origin` 403. Original note follows. ~~`ProbeGuard` "is the listener loopback-bound" input: **resolved by Task 1.2.3p (first task of Story 1.2.3)**, which reads the listen-address config in `main.go`/`server.go` and `config/config.go:212-214` (default `localhost:8543` at `:848`) and records the accessor and the exact `Addr` string the deployed instance uses; until then the accessor is UNVERIFIED. Wiring point is now decided (Start() chain only, Story 1.2.3); only this accessor remains open. **Reachability note (pre-mortem #3):** if the UI is reached on `:8543` through a LAN or Tailscale hostname, that name must be allowed via `SetHostnames`, or the guard 403s every probe. The guard rejection must render as "Couldn't check right now." (never "not found"), which is covered by the hook's `PermissionDenied`/403 mapping test (Task 2.1.1b) and by `TestProbeGuard_should_Pass_When_HostPublishedViaSetHostnames` (Task 1.2.3b). Task 1.2.3p records which Host values the deployed instance's guard sees (manual instance with `STAPLER_SQUAD_INSTANCE`); log the rejecting reason and Host at Warn.
- [ ] **Measured `--help` timings** (Flagged Choice 11): the timing table is UNMEASURED until Task 1.0.1 (first task) runs; the timeout decision (keep 3s / `slowTools` override / `PENDING` status) is made there and recorded in that table. Blocks Task 1.1.4b's final TTL/limit values and, if it yields `PENDING`, Task 1.2.1a.
- [ ] **`NEEDS_CONFIRM` first-check cost** (Flagged Choice 12): claude, gemini and aider are shebang scripts, so each needs one explicit Check the first time. Pending decision H3; needs Tyler's acceptance together with ADR-001 sign-off (PENDING). The picker honours the confirmed set, so already-checked programs do not regress after cache TTL; they do after a server restart or a changed mtime/size, because the confirmed set is in memory (persisting execution consent to disk is a security-model change, deferred); if rejected, the fallback is an allow-list of known CLI basenames that skip confirmation, which is a security-model change and must go through ADR-001.
...
```
1.0.1 timing go/no-go (FIRST; may amend 1.2.1a proto before proto-gen)
   |
   v
M1 walking skeleton (resolve + found/not-found; no --help execution)
 1.1.1 resolve -> 1.1.4a Probe part 1 -> 1.1.4d/d2/e login PATH
 1.2.1 proto -> 1.2.2 handler -> registry ; 1.2.3p -> 1.2.3 ProbeGuard ; 1.2.4 kill switch
   |  (proto-gen makes TS client available)
   v
 Phase 2: 2.1.1 hook -> 2.1.2 badge -> 2.2.1 ProgramsManager
                                   \-> 2.3.1 ProgramOption ext -> 2.3.2 ProgramProbeSection -> panel (1 line)
   |
   v  MVP cut line ------------------------------------------------------
Gate G1: 1.1.3a-c fixtures + parser; continue only if >=3 of 5 targets parse >0 flags
   |
   v
M2: 1.1.2 runner (+1.1.2e gosec) -> 1.1.4b cache/flight -> 1.1.4f/g gate -> 1.1.4a2 Probe part 2 -> 2.1.2c copy switch-on
 Phase 3 (autocomplete): 3.1.1 -> 3.1.2 FlagCombobox -> 3.1.3 wire
 Phase 4 (warnings, descriptions, AC9): 4.1.1 -> 4.1.2 ; 4.2.1 -> 4.2.2 -> 4.2.3a-b (AC9 touch e2e, gates Phase 4 release)
 Phase 5: 5.1b desktop e2e -> 5.2.1a gates (incl. make security, jscpd) -> 5.2.1b manual screen-reader checklist
```
...
| AC | Tasks |
|----|-------|
| 1 (RPC, fields) | 1.2.1a, 1.2.1b, 1.2.2a, 1.2.2c, 1.2.2e, 1.1.4a, 1.1.4a2, 1.2.4a |
| 2 (LookPath, `~`, found=false not error) | 1.1.1a, 1.1.1b, 1.1.4a, 1.1.4c, 1.1.4d, 1.1.4d2, 1.2.2c |
| 3 (3s timeout, no stdin, 256KB cap, no shell, cache by path+mtime) | 1.0.1 (timing go/no-go, first task), 1.1.2a-d, 1.1.3a (smoke test), 1.1.4a-c |
| 4 (parser, aliases, empty not error) | 1.1.3a-d, 1.2.1a |
| 5 (Program Config indicator on blur) | 2.1.1a-b, 2.1.2a-b, 2.2.1a-b, 4.2.3a-b, 5.1b |
...
| 8 (security; timeout and oversize tests; scripts need explicit Check) | 1.1.1a-b, 1.1.2a-e (incl. gosec G204), 1.1.4b-g, 1.2.1a, 1.2.3p, 1.2.3a-b, 1.2.4a-b (kill switch), 2.1.1a-b, 2.3.2a, 4.2.3a, 5.1b, 5.2.1a, ADR-001, ADR-002 |
| 9 (mobile tap, no hover) | 4.2.1a-b, 4.2.2a, 3.1.2a (44px rows), 4.2.3a-b (**touch e2e is the AC9 proof, shipped with the control in Phase 4**; jest is a unit substitute only) |
| 10 (Go parser fixtures; jest tests) | 1.1.3a-d, 2.1.1b, 2.1.2b, 2.2.1b, 2.3.2c, 3.1.2b, 4.1.1b, 4.2.1b, 4.2.3a-b, 5.1b |
...
- **AC5**: *Given* Program Config with `prog-command-input` focused and `probeProgram` mocked to return `{found:true, resolvedPath:"/usr/bin/claude", probeStatus:FOUND_PARSED}`, *When* the user types `claude` and blurs the field (or presses Enter, which runs the same implicit probe immediately, or clicks `prog-command-check`), *Then* `prog-command-status` (role=status) reads "Found: /usr/bin/claude" with a check icon and the detail "Checked on this server only"; for `NOT_FOUND` it reads "Not found as an executable on the server's PATH. Shell aliases and functions are not checked; the program may still work when launched from your shell." and Save remains enabled; Enter does not submit the form; *And* given a result of `NEEDS_CONFIRM` (script), *Then* the badge reads "Found: <path>. Not checked for flags yet. Check runs `<program> --help` on this server." until `prog-command-check` is clicked; blur and Enter never run it (Enter on a script leaves the same hint showing and does not submit the form); after the click the `prog-command-check` node is the same element, still focused, `disabled` with `aria-busy="true"` while the run is in flight, and re-enabled (label "Check again") when it settles.
...
Phase 1 is split by the milestones above. **M1 tasks**: 1.0.1, 1.1.1a-b, 1.1.4a, 1.1.4d, 1.1.4d2, 1.1.4e, 1.2.1a-b, 1.2.2a-e, 1.2.3p, 1.2.3a-c, 1.2.4a-b. **G1 spike** (run right after M1, costs about 9h, decides whether M2 happens): 1.1.3a-c. **M2 tasks** (only if G1 passes): 1.1.2a-e, 1.1.3d, 1.1.4a2, 1.1.4b-c, 1.1.4f-g. The hour table below counts 1.1.3a-c under "M2 part", so the go/no-go spend is 1.1.3a-c on top of M1. Task IDs are unchanged from earlier drafts where they existed; new IDs (1.0.1, 1.1.2e, 1.1.4a2, 1.1.4d2, 1.2.3p, 1.2.4a-b) were added by the triad repair.

### Epic 1.0: Measure before building
**Goal**: Decide the timeout branch (and whether M2 is worth building) before any proto or runner code exists.

#### Story 1.0.1: Timing go/no-go
**As an** implementer, **I want** measured `--help` wall times and side effects for the real target CLIs, **so that** the `Limits`, the `ProbeStatus` enum and the M2 decision rest on data, not the INFERRED 3s guess (pre-mortem P1-1).
**Acceptance Criteria**:
- *Given* the five targets on the dev machine, *When* Task 1.0.1 finishes, *Then* the "Measured `--help` timings" table has no UNMEASURED cell for installed targets (missing ones are named), and the Decision line names one branch: default 3s kept, `slowTools` override, or `PENDING` status.
**Files**: this plan (timing table only; no code)

##### Task 1.0.1: Timing, `$HOME` and man measurement (3h) - FIRST TASK
...
- **Also record** (same runs): whether any target wrote under `$HOME` (Unresolved Questions #6), and whether `git --help` under `PAGER=cat MANPAGER=cat` spawns `man`.
- **Branch consequences.** `slowTools` only: recorded here, applied in Task 1.1.2b. `PENDING` status: the enum value is added to Task 1.2.1a's proto text **before** `make proto-gen` (Task 1.2.1b), and Tasks 2.1.1a/2.1.2a gain a `pending` state; do this before any UI work.
- Files: this plan's timing table only (no code)
...
##### Task 1.1.1a: Implement `Resolve` (2h)
...
##### Task 1.1.2b: `runWith` seam + `Run` (4h)
- Types: `Limits{Timeout time.Duration; MaxBytes int}`, `DefaultLimits()` (3s, 256<<10), `RunFunc` (glossary). `limitsFor(base Limits, basename string) Limits` applies the `slowTools` override (empty until Task 1.0.1's measurement justifies entries); `Prober` passes its result to `run`. Unexported `runSpec{path ResolvedPath; args []string; extraEnv []string; parentEnv []string; inheritEnv bool; lim Limits}`. `parentEnv` is the environ `runWith` feeds to `probeEnv` (nil means `os.Environ()`, the production value); tests set it to a poisoned list so the no-leak proof runs through the real runner. `inheritEnv` is used only by `deriveLoginPath` (Task 1.1.4d).
...
##### Task 1.1.2d: Runner tests (3h)
- **Timing rule for this and every timing-shaped test in the plan (AC3's 80ms/500ms/1s/100ms figures, the 30s/10min fake-clock TTLs excepted): no fixed tight wall-clock bound.** Use `testutil/wait` from commit `d722197683a5` (`git show --stat d72219768`): `wait.RequireEventually(t, cond, baseTimeout, tick)` for "eventually" facts and `wait.ScaleTimeout(base)` for any upper bound (it stretches by measured load average, capped 8x, and honours `STAPLER_SQUAD_TEST_TIMEOUT_SCALE`). The load-independent assertions are the real proof (`TimedOut==true`, `Truncated==true`, process group gone via `RequireEventually` on `Kill(-pid,0)==ESRCH`); the wall-clock bound is only a scaled sanity check. This addresses the BUG-090..099 "under full-suite load" flake family (engineering triad finding).
...
##### Task 1.1.2e: gosec G204 handling for the two exec sites (1h)
- `runWith` executes a user-derived absolute path and `deriveLoginPath` runs `$SHELL -c <constant script>`; gosec flags both as G204. Add a targeted `//nolint:gosec // G204: <path> is a Resolve/checkExecutable-validated absolute regular file, argv is the constant --help` (and, for the shell, `// G204: script is a package constant, $SHELL is the server's own login shell`) on exactly those two `exec` calls, matching how commit `cc31acf8c` (13 G204 sites) handled the rest of the repo. Never a file-level or package-level suppression. Run `make security` and `make lint`; both must be green with the annotations present. Also confirm the custom analyzers under `tools/lint` (for example `norawgitopen`) do not flag the new package.
- Files: `config/clihelp/runner.go`, `config/clihelp/loginpath.go`
...
##### Task 1.1.3a: Capture fixtures (3h)
- Run real `--help` (stdout+stderr merged, `NO_COLOR=1`, `PAGER=cat`, `MANPAGER=cat`) for: `claude` (commander), `aider` (argparse), `gh`, `gh pr list` (cobra), `rg` and `uv` (clap), `gemini` (yargs), `agy` (Go `flag`), plus negatives `tmux` and `git` (see Unresolved Questions). Name `<tool>-<version>.txt`; add one ANSI-coloured variant. Capture command noted in a `parser_test.go` comment only.
- Timing, `$HOME` and `git`/`man` observations are recorded earlier, in Task 1.0.1. Which of the five targets are shebang scripts (`head -c2`) is also recorded there.
...
##### Task 1.1.3c: Golden and negative tests (2h)
...
##### Task 1.1.3d: Fuzz and adversarial (1.5h)
- `FuzzParseHelp` (no panic, bounded result, name-regex invariant) plus adversarial cases (256KB of `-`, no newlines, 10k tiny lines) with a 100ms base bound passed through `wait.ScaleTimeout` (Task 1.1.2d rule), plus a linear-scaling check (2x input takes less than about 4x work, measured by counted steps or `testing.AllocsPerRun`, not wall time) so a quadratic parser is caught without a flaky clock.
...
##### Task 1.1.4a: `Prober` pipeline, part 1: resolve, lookup, stat (3h) - M1
...
##### Task 1.1.4c: Prober tests (4h) - M1 lookup cases, M2 the rest
...
##### Task 1.2.1a: Edit proto (2h)
- **Precondition**: Task 1.0.1's Decision line is recorded. If it chose `PENDING`, add `PENDING = 8` to `ProbeStatus` below now, before Task 1.2.1b runs `make proto-gen`.
...
##### Task 1.2.1b: Generate and build (1h)
...
##### Task 1.2.2a: Handler + prober field + setter (2h)
...
##### Task 1.2.2b: Delegating wrapper (1h)
...
##### Task 1.2.2c: Handler tests (2h)
- **Placement (BUG-067 / BUG-099)**: `server/services` is the package whose `-race` suite already exceeds the MCP CI job budget (`docs/bugs/open`), so keep new tests there to the minimum that needs `DefaultsService` (this file, plus the hermetic-constructor and end-to-end tests); everything else (prober, cache, gate, login PATH, runner, parser) lives in `config/clihelp`, which is its own package. New `goleak` tests must be hermetic (no real shell, no network).
...
##### Task 1.2.2e: End-to-end handler test with real Prober (1h)
...
##### Task 1.2.3p: Resolve the bound-address accessor (Unresolved Q #7) - FIRST task of Story 1.2.3 (1h)
- Answer to verify: how the guard learns whether the listener is loopback-bound. Read the listen-address config in `main.go` and `server/server.go` (where `httpServer.Addr` is set from `config.ListenAddress`, `config/config.go:212-214`, default `localhost:8543` at `:848`), decide the accessor (for example `func (s *Server) ListenAddr() string`), and record: (a) the accessor name and its return for the deployed default, (b) that the hostname `localhost` counts as loopback (parse with `net.SplitHostPort`, IPv6 brackets), (c) which `Host` values the deployed instance sees when the UI is opened locally, via a manual instance (`STAPLER_SQUAD_INSTANCE=claude-manual-test`, `PORT=62871`, never the live `:8543`). Write the result into Unresolved Question #7 (mark it resolved) before Task 1.2.3a starts.
- Files: none committed (plan note); accessor added in Task 1.2.3a

##### Task 1.2.3a: Middleware + wiring (4h)
...
##### Task 1.2.3c: Per-listener chain tests (2h)
...
#### Story 1.2.4: Live-settable kill switch (Flagged Choice 8)
**As an** owner, **I want** to turn probing off from Settings without a deploy, **so that** a misbehaving probe or a rejected ADR-001 can be neutralized immediately.
**Acceptance Criteria**:
- *Given* flag `programs:cli-flag-probe` (default ON) toggled off through `UpdateFeatureFlag`, *When* `ProbeProgram` is called, *Then* it returns `connect.CodeUnimplemented` on the next request (no restart); *Given* it is ON or never persisted, *Then* the call proceeds; *Given* it is off, *Then* the hook reports `disabled` and Program Config and the picker render no badge, combobox suggestions or warnings (plain inputs; the removed `isProgramRecognized` span is not restored).
**Files**: `server/services/feature_flag_service.go`, `server/server.go`, `web-app/src/app/settings/features/page.tsx`, tests

##### Task 1.2.4a: Flag, interceptor, tests (3h) - M1
- Before coding, read how the closest existing flag is defined and enforced (`knownFeatureFlags` in `server/services/feature_flag_service.go`, `server/server.go:678`, `config.GetFeatureFlagWithDefault`) and copy that shape. Add `{name: "programs:cli-flag-probe", description: "...", defaultValue: true}` to `knownFeatureFlags`; add a `NewScopedFeatureFlagInterceptor("programs:cli-flag-probe", func() bool { return config.LoadConfig().GetFeatureFlagWithDefault(name, true) }, "ProbeProgram")` to the handler options next to the existing registration at `:678` (evaluated per request, so a toggle applies immediately). Tests in `server/services/feature_flag_service_test.go` style (reuse existing helpers): default on, off -> `Unimplemented`, on again -> passes. No env var anywhere.
- Files: `server/services/feature_flag_service.go`, `server/server.go`, `server/services/feature_flag_service_test.go`

##### Task 1.2.4b: Settings entry and UI mapping (1h) - M1
- Add a `FEATURE_META` label for `programs:cli-flag-probe` in `web-app/src/app/settings/features/page.tsx` (extend `page.test.tsx` only if it asserts the full list). The hook maps `Code.Unimplemented` to an idle "disabled" state that renders nothing (Task 2.1.1a); no client-side flag read is needed.
- Files: `web-app/src/app/settings/features/page.tsx`, `web-app/src/app/settings/features/page.test.tsx`
...
##### Task 2.1.1a: Hook (3h)
- `useProbeProgram(command: string, mode: "program-config" | "picker")` returns `{state: ProbeUiState, check(opts?: {explicit?: boolean}): void}`. Request fields: `explicit` (the Check button only; Enter calls `check()` without it) sends `confirm_execute: true`; blur/selection sends neither; `mode: "picker"` always sends `resolve_only: true` unless `explicit` (then `confirm_execute: true`, `resolve_only: false`). `status NEEDS_CONFIRM` maps to `needsConfirm{path}` (never memoized; an explicit check replaces it). `check` ignores empty command, skips if unchanged and settled (except an explicit call, or an immediate Enter, which always sends), ignores a second call while one is in flight for the same command (double-submit guard), maps `Code.Unimplemented` (kill switch off, Task 1.2.4a) to a `disabled` state that renders nothing, increments a request-token ref, aborts the previous `AbortController`, calls `client.probeProgram({command}, {signal})` (client per `ProgramsManager.tsx:77`), keeps a per-command in-memory memo (never memoizes `busyOrError`/`transportError`), resets to `checking` on command change, aborts on unmount. `// +feature: settings-programs` in first 10 lines.
- Files: `web-app/src/lib/hooks/useProbeProgram.ts`

##### Task 2.1.1b: Hook tests (3h)
...
##### Task 2.1.2a: Component (4h)
- Props `{state, checkedToken, onRetry, onConfirm, testId}`; renders every `ProbeUiState` variant per `research/ux.md` section 4 plus the wrapper, busy and `needsConfirm` variants above (`needsConfirm` shows a 44px `Check` button calling `onConfirm`; below 640px the button is a full-width 44px row under the text, at 640-1024px it sits inline after the text and wraps rather than truncating); the `Check` button is **kept mounted** for the lifetime of the command once shown (relabelled "Check again") so keyboard focus never lands on a removed node, is `disabled` with `aria-busy="true"` while `checking`, and results never move focus (UX-6); the "Checking..." text in the `role=status` region is announced only if the run lasts longer than 300ms (debounced render of the announcement text, no second announcement for a fast probe), while the visible spinner shows immediately; Retry button 44px on `transportError`/`busyOrError`; `busyOrError` and `transportError` both lead with "Couldn't check right now."; `transportError` adds one sentence noting the server may refuse probes when it listens on a non-loopback address without auth; static "Checking..." under reduced motion; `warning`/`warningBg`/`warningText` tokens (`styles/theme.css.ts:100-102`); "Checked: <token>" and full path on expand; `// +feature:` header.
- Files: `web-app/src/components/ui/ProbeStatusBadge.tsx`, `web-app/src/components/ui/ProbeStatusBadge.css.ts`

##### Task 2.1.2b: Badge tests (2h)
...
##### Task 2.1.2c: M2 copy switch-on (1h) - M2
- When `--help` execution lands (Task 1.1.4a2), `FOUND_NO_FLAGS` gets its "Couldn't read flags from --help; flag suggestions unavailable." sentence and `needsConfirm` becomes reachable for real scripts. Until then (M1) the badge shows only "Found: <path>" for `FOUND_NO_FLAGS`. One conditional on a `helpExecutionAvailable` constant flipped in this task; update the badge tests for both copies.
- Files: `web-app/src/components/ui/ProbeStatusBadge.tsx`, `web-app/src/components/ui/ProbeStatusBadge.test.tsx`
...
##### Task 2.2.1a: Wire (4h)
- At `ProgramsManager.tsx:313-321` add `onBlur={() => check()}` (implicit: never confirms a script), `onKeyDown` Enter -> `preventDefault(); check()` (D1 as changed: Enter runs an immediate implicit probe, which for a script only shows the NEEDS_CONFIRM hint and never sends `confirm_execute`; consent to run a script is the Check button alone), a `prog-command-check` `onClick={() => check({explicit:true})}` (kept mounted, `disabled` + `aria-busy` while checking, focus stays on it), and pass `onConfirm` from the badge's `Check` control (also explicit), `autoCapitalize="off" autoCorrect="off" spellCheck={false}` on `prog-command` and `prog-flags` (`:326-334`), a 44px `prog-command-check` button (D5), and `<ProbeStatusBadge testId="prog-command-status">` under the field with `aria-describedby`. Do not move focus.
- Files: `web-app/src/components/settings/ProgramsManager.tsx`, `web-app/src/components/settings/ProgramsManager.css.ts`

##### Task 2.2.1b: Tests (3h)
- Extend `__tests__/ProgramsManager.test.tsx` with the shared mock: blur -> found, not-found copy, transport error, busy, wrapper, Check button, Enter runs an immediate implicit probe and does not submit; Enter on a script shows the hint and sends no `confirm_execute`; Check button stays mounted and focused, is disabled with `aria-busy` during a run, and a second click during a run sends no second request; the `Checking...` announcement is debounced (fake timers: absent at 100ms, present after 300ms); Save enabled.
...
##### Task 2.3.1a: Extend type and mapper (1h)
...
##### Task 2.3.2a: `ProgramProbeSection` component (3h)
- Props `{option: ProgramOption | undefined}`. Owns `useProbeProgram(option?.command, "picker")`, the selection-change effect calling `check()` (resolve-only: **never executes a program on selection**; not per keystroke), a `Check` control (via the badge's `onConfirm`, same kept-mounted / full-width 44px row at 375px behaviour as Task 2.1.2a) that calls `check({explicit:true})`, **the picker relies on the server honouring the confirmed set for `resolve_only` (Task 1.1.4f)**, so an already-checked program shows its flags (or found) on selection instead of `NEEDS_CONFIRM` after the cache TTL; it does not carry its own client-side consent record, `ProbeStatusBadge`, and (task 4.1.2b) the saved-flag warning. Preserves `data-testid="preset-program-warning"` on the not-found variant. Behavior when the RPC is unavailable (`transportError`): falls back to nothing (the old `isProgramRecognized` list-membership check at `OmnibarCreationPanel.tsx:350` is removed with the span it fed). `// +feature:` header.
- Files: `web-app/src/components/sessions/ProgramProbeSection.tsx`

##### Task 2.3.2b: Panel wiring (1h)
...
##### Task 2.3.2c: Tests (3h)
- `ProgramProbeSection.test.tsx` reusing the shared probe mock: not-found, found, wrapper, busy, and no duplicate warning; selection sends `resolve_only:true` and never `confirm_execute`; the `Check` control sends `confirm_execute:true`; a `NEEDS_CONFIRM` result shows the badge and no flag warning; a mocked `resolve_only` response of `FOUND_PARSED` for a previously confirmed program (server behaviour) renders flags/warning with no Check prompt; at 375px the Check control is a full-width 44px row; and the on-screen-keyboard case (visual viewport shrunk to about 300px) keeps the Check row and badge reachable via `scrollIntoView`. Update `OmnibarCreationPanel.test.tsx` only for the removed span.
...
- ARIA 1.2 behaviour (AC6, AC9)
  - *Given* flags `[--model, --verbose]` and value `--mo`, *When* the user presses Down, *Then* the input has `role="combobox"`, `aria-expanded="true"`, `aria-controls`, `aria-autocomplete="list"`, `aria-activedescendant` = option id of `--model`; Escape closes without clearing; **Tab never accepts an option unless the user has moved to it with Down/Up** (Enter accepts; with no arrow navigation Tab just moves focus and the typed text is untouched, per the ARIA APG combobox pattern); option rows are at least 44px; tapping an option does not blur-trigger a re-probe (`onMouseDown` preventDefault); options contain no interactive children.
...
##### Task 3.1.3a: Wire (2h)
- Replace the plain input at `ProgramsManager.tsx:326-334` with `FlagCombobox` (keep `id="prog-flags"` and `data-testid="prog-flags-input"`); pass `flags` (empty when `isWrapper`); **flags-field hint**: in the `needsConfirm` state render a visible hint under the field, referenced by `aria-describedby`, "Suggestions appear after Check reads the flags." (and, when no probe has run, the existing "Check the command above" hint), so the absence of suggestions is explained in every no-flags state; add test cases including the hint in `needsConfirm`.
- Files: `web-app/src/components/settings/ProgramsManager.tsx`, `web-app/src/components/settings/__tests__/ProgramsManager.test.tsx`
...
##### Task 4.1.2a: Program Config warning (2h)
...
##### Task 4.1.2b: Session creation warning (2h)
...
##### Task 4.2.1b: Unit tap test (1h)
- `fireEvent.click`/`userEvent` only (no `mouseover`); assert expand/collapse and 44px min size via style tokens. This is a unit substitute; AC9 is proven by task 4.2.3b.
...
#### Story 4.2.3: AC9 touch proof ships with the control
**As a** maintainer, **I want** the AC9 touch e2e to land in the same phase as `FlagInfoButton`, **so that** Phase 4 is not shippable with AC9 unproven (engineering triad finding: the proof used to be Task 5.1b, a phase later).
**Acceptance Criteria**:
- *Given* the isolated e2e server and the shebang fixture confirmed via Check, *When* the touch context (`viewport 375x667`, `hasTouch: true`, `isMobile: true`) taps `prog-flag-info-button`, *Then* the description shows inline, `aria-expanded="true"` and the bounding box is at least 44x44. Phase 4 is not released until this passes.
**Files**: `tests/e2e/pages/ProgramsSettingsPage.ts`, `tests/e2e/fixtures/probe-fixture.sh`, `tests/e2e/cli-flag-discovery.spec.ts`

##### Task 4.2.3a: Playwright page helper + fixture (moved here from 5.1a) (3h)
...
##### Task 4.2.3b: AC9 touch spec, mid-width and keyboard cases (3h)
- Create `tests/e2e/cli-flag-discovery.spec.ts` (first line `// @feature program_config:probe, settings-programs`; no `waitForTimeout`; locators by `data-testid`/ARIA only) with: (1) **the AC9 proof**: touch context 375x667, Check the fixture, type `--alp`, tap `prog-flag-info-button` in the "Available flags" disclosure, assert `aria-expanded="true"`, description visible, bounding box >= 44x44; (2) a 640-1024px case (800x1024): Check control inline after the text, no horizontal scroll, listbox not clipped; (3) an on-screen-keyboard case at 375x667 with the visual viewport reduced (Playwright viewport 375x300 to emulate the keyboard): the flags combobox list and Save remain reachable and the active option is scrolled into view; (4) at 375px the NEEDS_CONFIRM `prog-command-check` is a full-width row at least 44px high. Run `cd tests/e2e && npx playwright test cli-flag-discovery.spec.ts`.
...
### Epic 5.1: Playwright
**Goal**: The desktop user path and server-side checks (AC5, AC8). The AC9 touch proof and its helper/fixture moved to Story 4.2.3.

#### Story 5.1.1: Program Config e2e
**As a** maintainer, **I want** the desktop e2e cases in the same spec, **so that** the wiring from blur to badge is guarded in CI (AC9 is already proven by Task 4.2.3b).
**Acceptance Criteria**:
- *Given* the isolated e2e server and a fixture script `probe-fixture` (created in the test dir, prints `--alpha  first flag` and `--beta <v>  second`) given by absolute path, *When* the test types its path in `prog-command-input` and blurs, *Then* `prog-command-status` has text "Found:" and "checked for flags yet" (the fixture is a shebang script, so nothing ran and the fixture's marker file does not exist); *When* `prog-command-check` is clicked, *Then* the marker exists, and typing `--alp` in `prog-flags-input` lists `--alpha`; a nonexistent path shows "Not found"; *And* pressing Enter in `prog-command-input` on the (unconfirmed) script leaves the marker absent and the hint showing, and does not submit the form. (The touch-viewport AC9 proof is Task 4.2.3b.)
**Files**: `tests/e2e/pages/ProgramsSettingsPage.ts`, `tests/e2e/cli-flag-discovery.spec.ts`, `tests/e2e/fixtures/probe-fixture.sh`

##### Task 5.1b: Desktop spec cases (4h)
- Extend `tests/e2e/cli-flag-discovery.spec.ts` (created in Task 4.2.3b) with the desktop cases: blur shows the hint and no marker; Check creates the marker and lists `--alpha`; Enter on the unconfirmed script shows the hint, no marker, no submit; nonexistent path shows "Not found"; badge and Check keep focus (`toBeFocused` on `prog-command-check` after the result); also a wrong-Host POST check via `request.post` with `Host: evil.example` expecting 403. Assertions use `expect(locator).toHaveText`/`toBeFocused`, no `waitForTimeout`.
- Files: `tests/e2e/cli-flag-discovery.spec.ts`
...
##### Task 5.2.1a: Run gates (3h)
- `make ready-complexity-gate`, `make ready`, `go test ./config/clihelp ./server/services ./server/middleware -race`, `pnpm run lint:duplicates` (in `web-app/`, pnpm only), and **`make security`** (gosec G204 annotations from Task 1.1.2e must hold). Fix any jscpd finding by extracting to the shared mock/helpers (`web-app/src/lib/hooks/__mocks__/probeProgramMock.ts` from Task 2.1.1b is the single probe mock; every new test file for the hook, badge, section, combobox and info button imports it instead of re-declaring `jest.mock(...)` blocks), not by raising the threshold; the repo's jscpd margin is thin (0.12% ceiling, recent PRs sit near it), so run the gate after each phase, not only at the end. Commit specific files only. No `docs/reference/program-probe.md` (dropped, Flagged Choice 9); ADRs are the record of the security envelope.
- Files: none new

##### Task 5.2.1b: Manual screen-reader checklist (1.5h)
- Axe cannot verify the moving `aria-describedby` on the combobox or the live-region announcements. On the manual dev instance (`PORT=62871`, `STAPLER_SQUAD_INSTANCE=claude-manual-test`, never the live `:8543`), run with NVDA (Windows/Linux Firefox) and VoiceOver (macOS Safari or iOS) and tick: (1) blur announces one status, not two for a fast probe; (2) NEEDS_CONFIRM hint and Check button are announced with name and role; (3) arrowing options announces name, takes-value and description; (4) Tab without arrowing does not alter text; (5) info button announces expanded/collapsed; (6) warning is announced via describedby and not as an alert. Record results in the PR description, not a new doc.
- Files: none
...
Still open after this pass (carried in Unresolved Questions): ADR-001 sign-off and the AC8/AC7 readings (PENDING human review); the measured timing table (UNMEASURED until Task 1.0.1, now the first task); acceptance of the first-check click cost for shebang-script CLIs (Flagged Choice 12); the guard bound-address accessor (Q #7).

## Triad repair log (product, UX, engineering; after `pm:triad-review`)

Source analyses: `triad-{product,ux,engineering}-cli-flag-discovery.txt` (session scratchpad). Nothing here records a human approval; H1-H3 in "Pending human decisions" remain PENDING.

| Lens | Finding | Resolution | Where |
|---|---|---|---|
| ENG 1 | Timing go/no-go ran after runner/proto; a `PENDING` outcome would ripple into proto and UI | New first task 1.0.1; states that `PENDING` amends the proto before `proto-gen` | Delivery order, Task 1.0.1, Task 1.2.1a precondition |
| ENG 2 | "3-6 min" labels unrealistic; 1.1.4a and 1.1.4d oversized | Every task headed with an hour estimate (1-4h); per-phase and total table (63 tasks, 151h, recounted by script); 1.1.4a split into 1.1.4a and 1.1.4a2; 1.1.4d split, `main.go` wiring is Task 1.1.4d2 | Task headings, Delivery order table |
| ENG 3 / PROD | Phase 1 front-loaded; no MVP cut line; no kill criterion | Milestone M1 (walking skeleton) then Gate G1 (continue only if >=3 of 5 targets parse >0 flags) then M2 | Delivery order, Phase 1 preface, Dependency Visualization |
| ENG 4 | AC9 proof was in Phase 5 while Phase 4 was shippable | Helper/fixture and touch spec moved to Story 4.2.3 (Tasks 4.2.3a-b); Phase 4 not released until they pass | Story 4.2.3, AC9, traceability |
| ENG 5 | No gosec G204 plan for exec of user-derived path and `$SHELL -c` | Task 1.1.2e (targeted `//nolint:gosec` with justification on two sites), `make security` in Task 5.2.1a | Task 1.1.2e, 1.1.4d, 5.2.1a |
| ENG 6 | Fixed tight timing thresholds; new tests in flaky `server/services` | All timing-shaped tests use `testutil/wait` `ScaleTimeout`/`RequireEventually` (commit `d722197683a5`); behavioural assertions are the proof; parser bound gets a step-count linearity check; tests kept in `config/clihelp` (BUG-067/BUG-099 note) | Task 1.1.2d, 1.1.3d, 1.2.2c, AC3 |
| ENG 7 | Unresolved Q #7 (bound-address accessor) | Concrete first task of Story 1.2.3, Task 1.2.3p, with the values to verify | Task 1.2.3p, Unresolved Questions |
| ENG 8 | No live-settable kill switch (repo rule: flags live-settable, never env vars) | Existing mechanism found and judged light enough (`knownFeatureFlags` + `NewScopedFeatureFlagInterceptor` at `server.go:678` + Settings `FEATURE_META`): flag `programs:cli-flag-probe`, default ON, Story 1.2.4 | Flagged Choice 8, Risk Control, Tasks 1.2.4a-b |
| ENG 9 | jscpd margin | One shared probe mock (`__mocks__/probeProgramMock.ts`) required for every new test file; run the gate per phase | Task 5.2.1a, Task 2.1.1b |
| PROD | Outcome checks, value-risk assumption, MVP cut line | Added to `requirements.md` Interpretation notes; cut line and kill criterion in plan | `requirements.md`, Delivery order |
| UX a | Picker regressed to `NEEDS_CONFIRM` after TTL/restart/mtime change | `resolve_only` honours the confirmed set (Task 1.1.4f), so TTL expiry no longer regresses; mtime/size change and server restart still re-prompt on purpose (different bytes; consent is in memory, persisting it is a deferred security-model change) | Flagged Choice 12, Tasks 1.1.4f-g, 2.3.2a, Unresolved Questions |
| UX b | NEEDS_CONFIRM copy used "script" jargon, no rationale | "Found: <path>. Not checked for flags yet. Check runs `<program> --help` on this server." | AC5, Task 2.1.2a, `design/ux.md` S6/S5, `validation.md` |
| UX c | Enter as consent to execute | Enter runs an immediate implicit probe only; on a script it shows the hint; only the Check button sends `confirm_execute` | Flagged Choice 9 (D1 changed), Tasks 2.1.1a, 2.2.1a, `design/ux.md` D1 |
| UX d | Tab accepted the active option (departs from APG) | Tab accepts only after the user arrowed to an option; Enter accepts | Story 3.1.2, Tasks 3.1.2a-b, `design/ux.md` S2 |
| UX e | Focus/busy/announcement unspecified | Check button kept mounted and focused, `disabled` + `aria-busy` during a run, double-submit guard, "Checking..." announcement debounced to >300ms | Tasks 2.1.1a, 2.1.2a, 2.2.1a, `design/ux.md` |
| UX f | No flags-field hint in NEEDS_CONFIRM | Hint "Suggestions appear after Check reads the flags." with `aria-describedby` | Task 3.1.3a, `design/ux.md` S2 |
| UX g | 375px layout, 640-1024px and keyboard cases, screen reader | Full-width 44px Check row at 375px; e2e for 640-1024px and reduced-height (keyboard) viewport; manual NVDA/VoiceOver checklist Task 5.2.1b | Tasks 2.1.2a, 2.3.2c, 4.2.3b, 5.2.1b, `validation.md` |

Not resolved here: the three PENDING human decisions (H1-H3); the timing table (UNMEASURED until Task 1.0.1); the guard accessor (until Task 1.2.3p). The `Proxy` (`env ... claude`) built-in program is a wrapper, so the M2 value for it is badge-only by design.
