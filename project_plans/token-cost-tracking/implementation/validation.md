...
| AC-5 | `docs/registry/coverage-gaps.json` (script-driven, no new test file) | `registry-generate_should_reportNoUnexplainedCoverageGapGrowth_When_runTwiceAfterEdits` | Verification | Run `make registry-generate` twice then `make registry-diff` (per the two-pass ordering finding in Risk Control); diff `unmatchedFrontend`/`unmatchedBackend` counts against the pre-change baseline |
