...
- [ ] **Pre-implementation gate**: as of 2026-08-01, the `end_reason` prerequisite commits (`0ac676001`, `f7ab0c9ad`) are committed locally but confirmed NOT merged into either `origin/main` or `upstream-fanatics/main` (verified via `git merge-base --is-ancestor`). Phase 1 must not start on a fresh worktree/branch cut from either remote's `main` until this is resolved — see Story 0.1.1 / Task 0.1.1a for the required remote-merge gate and the two acceptable resolutions (merge upstream first, or explicitly flag the unmerged dependency in the PR description if implementing in the same local branch). Re-check this at Phase 1 kickoff; if still unresolved by then, treat it as blocking, not a soft reminder — owner: implementer, resolve before Phase 1, not inline.
...
**As a** planner/implementer, **I want** to confirm `end_reason`'s backend plumbing is fully committed AND merged to both canonical remotes, **so that** Phase 1's proto/frontend work isn't built on a moving target or on history that a fresh worktree/branch cut from either remote's `main` won't actually have.
**Acceptance Criteria**:
- Confirmed via `git status --short` and `git diff` on `session/ent/schema/item_session.go` and `server/services/backlog_service_triage.go` that neither file has uncommitted changes touching `end_reason`.
  - *Given* the repo working tree at `/home/tstapler/Programming/stapler-squad`, *When* running `git status --short session/ent/schema/item_session.go server/services/backlog_service_triage.go`, *Then* the output is empty (no uncommitted changes) — confirmed during planning research (2026-08-01): both commands returned empty, and `end_reason` is present in `session/ent/schema/item_session.go`, introduced locally by commit `0ac676001`.
- Confirmed via `git merge-base --is-ancestor` that the commit(s) introducing `end_reason` are ancestors of **both** `origin/main` and `upstream-fanatics/main` (this repo's two canonical remotes per CLAUDE.md), not just present on the local working copy's `main`.
  - *Given* commits `0ac676001` and `f7ab0c9ad` (the commits that added `end_reason` plumbing per this plan's research), *When* running `git merge-base --is-ancestor 0ac676001 origin/main`, `git merge-base --is-ancestor 0ac676001 upstream-fanatics/main`, and the same two checks for `f7ab0c9ad`, *Then* all four checks exit 0 — **confirmed during planning/review (2026-08-01): all four currently fail (exit 1)**. Both commits exist only on this working copy's local `main`, ahead of both `origin/main` and `upstream-fanatics/main`. This is a **confirmed blocker, not a hypothetical** — see Task 0.1.1a for the required gate.
...
- **Remote-merge gate (required, not optional)**: identify the commit that actually introduced `end_reason` to `session/ent/schema/item_session.go` at implementation time via `git log --oneline -- session/ent/schema/item_session.go` (the SHA may differ from `0ac676001`/`f7ab0c9ad` by the time Phase 1 starts — use whatever commit currently touches that field). Then run:
  - `git merge-base --is-ancestor <commit> origin/main`
  - `git merge-base --is-ancestor <commit> upstream-fanatics/main`
  - Do the same for whichever commit added the corresponding backend plumbing in `server/services/backlog_service_triage.go` (or its current equivalent file, per `git log`).
- **If any of these checks fail** (commit not yet an ancestor of one or both remotes), this is a hard stop, not a soft note — do not proceed with Phase 1 on a fresh worktree/branch cut from either remote's `main`. Choose one:
  - (a) Push and merge the prerequisite commit(s) to both `origin/main` and `upstream-fanatics/main` first, then re-run this check before starting Phase 1; or
  - (b) If Phase 1 is being implemented directly in the same local branch/working copy that already contains the prerequisite commits (i.e. not a fresh worktree cut from a remote), it is safe to proceed there, but the resulting PR description must explicitly call out that Phase 1 depends on an unmerged prerequisite (naming the commit SHA(s)) so reviewers and any other worktree/session know not to assume `end_reason` exists upstream yet.
...
- **Do not recolor this badge** to match the stuck-reason red/orange severity scale (`vars.color.warningBg`/`errorBg` etc.) — a pause is not a failure and may be entirely manual/intentional; keep the badge's existing neutral/informational styling (per ux.md's Cross-surface consistency notes, "One severity vocabulary, five surfaces").
- Files: `web-app/src/components/sessions/SessionCard.tsx`
...
- `BlockerChip` renders `next_remediation_at` as a short relative-time-until string (e.g. "retries in 2h") appended to the chip's `aria-label`, when `0 < remediationAttempts < 5` (still actively retrying, not parked) and `nextRemediationAt` is set.
  - *Given* a `StuckBacklogItem` with `remediationAttempts: 2` and `nextRemediationAt` set to a timestamp 2 hours in the future, *When* the chip renders, *Then* its `aria-label` includes retry-timing text (e.g. `"Respawned 2 times — retries in 2h"`), computed by a new relative-time-until formatter (see Task 3.1.1d). No existing formatter renders `next_remediation_at` today — research confirmed `/unfinished`'s `StuckItem.tsx` never surfaces this field either, so the new formatter mirrors `stuckReason.ts`'s `formatAgo`/`formatStuckDuration` `timestampToMs()`-based mechanism rather than reusing a nonexistent one.
  - *Given* a parked chip (`remediationAttempts >= 5`), *When* it renders, *Then* no retry-timing text is appended — a parked item has no scheduled next remediation, only the existing "remediation attempts exhausted" wording applies.
- `BlockerChip`'s `variant="full"` rendering additionally surfaces `item.context` (when non-empty) via the chip's tooltip/`aria-label`, not as a new always-visible text line — avoiding a third always-visible text element alongside the existing icon+label and duration (per pitfalls.md's over-disclosure guidance).
  - *Given* a `StuckBacklogItem` with `context: "3 consecutive timeout failures on headless triage"`, rendered via `<BlockerChip variant="full" item={stuckItem} />` on `LifecycleSummary.tsx`, *When* the chip renders, *Then* the context text is available via the chip's tooltip/`aria-label`, but no new always-visible `<span>` is added to the compact layout.
  - *Given* `item.context` is empty/unset, *When* the chip renders, *Then* no context text is added to the tooltip/`aria-label` (unchanged from today).
**Files**: `web-app/src/components/backlog/BlockerChip.tsx`, `web-app/src/components/backlog/BlockerChip.css.ts`, `web-app/src/components/backlog-stuck/stuckReason.ts`

##### Task 3.1.1a: Add the `×N` suffix span (~4 min)
- In `web-app/src/components/backlog/BlockerChip.tsx`, insert the `×N` suffix span immediately after the chip's label element and **before** the existing `variant === "full" && <span className={styles.duration}>...</span>` block — matching ux.md's Surface 4 wireframe DOM order (`label, ×N, duration`, not `label, duration, ×N`), which also produces the correct screen-reader reading order: `{item.remediationAttempts > 0 && <span className={styles.remediationCount} aria-label={\`Respawned ${item.remediationAttempts} time${item.remediationAttempts === 1 ? "" : "s"}${item.remediationAttempts >= 5 ? " — remediation attempts exhausted" : ""}\`} data-testid="blocker-chip-remediation-count">×{item.remediationAttempts}</span>}`. The existing duration block continues to render immediately after this new span, unchanged.
- Files: `web-app/src/components/backlog/BlockerChip.tsx`

##### Task 3.1.1d: Add `next_remediation_at` retry-timing text (~4 min)
- Add a new `formatRetryCountdown(nextRemediationAt: Timestamp | undefined, now?: number): string` helper to `web-app/src/components/backlog-stuck/stuckReason.ts`, alongside `formatAgo`/`formatStuckDuration` — mirror their `timestampToMs()`-based conversion but compute a *future*-relative duration (e.g. `"retries in 2h"`, `"retries in 45m"`), returning `""` when `nextRemediationAt` is unset or already in the past.
- In `web-app/src/components/backlog/BlockerChip.tsx`, when `0 < item.remediationAttempts < 5`, append the non-empty result of `formatRetryCountdown(item.nextRemediationAt)` to the `aria-label` built in Task 3.1.1a (join with `" — "`).
- Files: `web-app/src/components/backlog-stuck/stuckReason.ts`, `web-app/src/components/backlog/BlockerChip.tsx`

##### Task 3.1.1e: Surface `context` in `BlockerChip`'s tooltip/aria-label (`variant="full"`) (~3 min)
- In `web-app/src/components/backlog/BlockerChip.tsx`, for `variant === "full"`, when `item.context` is non-empty, append it to the chip's existing hover-text mechanism (`title` attribute or `aria-label`, whichever the chip currently uses — check the implementation first), e.g. `... — ${item.context}`. Do not render `item.context` as a new always-visible `<span>`.
- Files: `web-app/src/components/backlog/BlockerChip.tsx`
...
- Before inserting, `CreateRespawnEvent` runs a bounded existence check and skips the insert (returning `nil`, not an error) if an equivalent row already exists within a short window — dedupe key `(item_id, triggering_session_uuid, reason)`, window ~10s — via `r.client.RespawnEvent.Query().Where(respawnevent.ItemID(parsedID), respawnevent.Reason(reason), respawnevent.TriggeringSessionUuid(triggeringSessionUUID), respawnevent.CreatedAtGTE(time.Now().Add(-respawnEventDedupeWindow))).Exist(ctx)`. This closes the exact duplicate-call class commit `0ac676001` fixed for `RemediateStaleWorkSession` (reconciliation sweeps firing the same respawn decision twice). A query-based check is used instead of a static unique index because `triggering_session_uuid` is unset (NULL) for 2 of the 4 call sites (`AutoRespawnReview`, `AutoRespawnTriage`), and SQL unique indexes treat NULLs as distinct — a static index would silently fail to dedupe exactly those two call sites.
  - *Given* two near-simultaneous calls to `CreateRespawnEvent(ctx, itemID, "stale_work_remediation", "stale-abc", "fresh-def")` for the same item within 1 second of each other (the exact race `0ac676001` demonstrates is real for `RemediateStaleWorkSession`), *When* both calls execute, *Then* only one `RespawnEvent` row exists in the table afterward — the second call's existence check finds the first call's row and skips the insert.
...
##### Task 4.2.1e: Add dedupe guard inside `CreateRespawnEvent` (~4 min)
- In `session/ent_repository_backlog.go`'s `CreateRespawnEvent` (Task 4.2.1c), before calling `.Save(ctx)`, add a `const respawnEventDedupeWindow = 10 * time.Second` near the top of the file, then run: `exists, existErr := r.client.RespawnEvent.Query().Where(respawnevent.ItemID(parsedID), respawnevent.Reason(reason), respawnevent.TriggeringSessionUuid(triggeringSessionUUID), respawnevent.CreatedAtGTE(time.Now().Add(-respawnEventDedupeWindow))).Exist(ctx)`.
- If `existErr != nil`, log it and proceed to the insert anyway (fail open — a broken dedupe check must never block the underlying write, matching this method's existing best-effort contract). If `exists` is `true`, log at Info level (e.g. `"skipped duplicate respawn event for item %s reason %s"`) and return `nil` without calling `.Save(ctx)`.
- Verify the generated predicate name for the nillable `triggering_session_uuid` field after Task 4.1.1c/4.2.1b (may be `respawnevent.TriggeringSessionUuid` or `respawnevent.TriggeringSessionUuidEQ` depending on ent's generated naming for optional fields) and use whichever `ent generate` actually produced.
- Files: `session/ent_repository_backlog.go`
...
- A test replays the double-invocation scenario commit `0ac676001` fixed and asserts exactly one `RespawnEvent` row results, not two (regression coverage for Task 4.2.1e's dedupe guard).
  - *Given* `RemediateStaleWorkSession`'s respawn path is invoked twice in quick succession for the same item and the same active session `SessionUUID: "stale-abc"` (simulating two overlapping reconciliation-sweep goroutines both observing the same stale state before either completes — the exact shape `0ac676001` fixed), *When* both calls run to completion, *Then* querying `respawn_events` for that item/reason afterward returns exactly 1 row, not 2.
...
##### Task 4.4.1d: Add double-invocation dedupe regression test (~5 min)
- Add a test in `server/services/backlog_service_triage_test.go` (or nearest existing test file for this handler — confirm exact filename via `Glob` first) that invokes `s.autoRespawnAutonomousWork(ctx, itemID, session.RespawnReasonStaleWork, "stale-abc")` twice back-to-back (or concurrently via two goroutines + a `sync.WaitGroup`, to match the reconciliation-sweep race shape `0ac676001` fixed) for the same item, then queries the `respawn_events` table (via `storage`/a test helper or a direct ent query against the test DB) and asserts exactly one `RespawnEvent` row exists for that item/reason — this is the regression test for the dedupe guard added in Task 4.2.1e.
- Run `go test ./server/services/...` to confirm it passes.
- Files: `server/services/backlog_service_triage_test.go` (or existing equivalent)
...
- The section's header/subtitle includes a brief caption clarifying that its count is all-time and distinct from the `×N` remediation-attempt count shown elsewhere on the same item (`BlockerChip`, Story 3.1.1) — e.g. "All-time respawn history — may exceed the current retry count shown above if this item has been resolved and re-stuck before." — so a user comparing the two numbers on the same page isn't left to guess at a mismatch.
  - *Given* a `BacklogItem` whose `stuckItem.remediationAttempts` is `2` (current stuck episode, rendered as `×2` on `BlockerChip` per Story 3.1.1) and whose `respawnEvents.length` is `7` (all-time respawn history, because the item previously resolved and re-stuck several times), *When* both `BlockerChip` and `RespawnHistorySection` render on the same item detail page, *Then* `BlockerChip` shows `×2` and `RespawnHistorySection` shows 7 rows plus the clarifying caption — with no error, warning, or implied contradiction between the two numbers.
