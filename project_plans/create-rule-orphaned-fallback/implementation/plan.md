...
| No-match | The `EscalationCategory` value meaning "no rule matched; classifier's default-escalate fallback applied" — the *only* kind of escalation that existed before PR #315 introduced the taxonomy | This is why treating "absent" the same as `"no-match"` is semantically correct, not just convenient |
...
| `orphanedCleanupThreshold` | 4-hour constant (`server/services/approval_store.go` L65) bounding how long an orphaned (session-gone) approval persists before cleanup | Not to be confused with "orphaned approval" in this plan's title — same word, different concept, per requirements.md's non-goals |
