...
| AC1: any resize-triggering event settles to zero further `fit()`/RPC calls within a bounded number of debounce cycles, across concurrently open sessions | None (no automated equivalent — 3 real browser tabs, CPU pegging, DevTools Performance panel) | Manual repro checklist, Story 5.2.1 | Manual | Given 3 concurrent browser tabs each with its own `XtermTerminal`/session, when the OS window is resized once (e.g. 1200×800 → 1400×900), then Chrome DevTools' Performance panel (5s recording from resize) shows the chain settling (no repeating `[XtermTerminal] Container resized` log lines after 1-2 debounce cycles) and CPU returns to idle (<5%) within 2s, in each tab. Outcome recorded in the PR description (pass/fail + trace/screenshot if fail). |
| AC7: original manual repro no longer pegs CPU or freezes input | None (same reasoning as AC1) | Manual repro checklist, Story 5.2.1 | Manual | Given the same 3-tab setup, when one tab is backgrounded/resumed or the window is resized once, then input in all 3 tabs remains responsive throughout (typing echoes without perceptible stall) and no tab freezes. Same checklist item as AC1, recorded in the PR description; do not skip even though it produces no test file. |
