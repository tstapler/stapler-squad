...
### Accepted Residual Risks (explicit sign-off, per Phase 4 triad review's engineering-lens findings)
These are deliberately *not* being fixed as part of this project — recorded here so the decision to defer is visible in the plan itself, not only in pre-mortem.md:
- **No domain-allowlist on `verifyHostnameOwnership`** (pre-mortem.md #2, P3): the gate only checks "resolves to one of my own IPs," not "resolves to an expected domain suffix." A hostile/compromised LAN DNS resolver could in principle plant a permanently-trusted RPID/TLS SAN. Accepted because: (a) rated P3 (unlikely) against this tool's actual network diet (home, a work location, occasional VPN — not open/public Wi-Fi), (b) an allowlist requires per-network configuration this single-developer tool doesn't otherwise have, and (c) the structured "dropped unverified candidate" log line (Epic 4.1) gives a way to audit after the fact if ever suspected.
- **Boot-time TLS SAN issuance stays unfiltered while the new runtime path is validation-gated** (adversarial-review.md, carried-forward concern): `startRemoteAccess`'s existing boot-time `networks` map is fed to `EnsureNetworkTLSCerts` without going through `verifyHostnameOwnership` today, and this project does not change that — only hostnames the *runtime* `HostnameDetector` discovers get the new gate. This pre-existing asymmetry is out of this project's scope to fix (it would mean changing `startRemoteAccess`'s current boot sequence, not just adding a new component) but is called out explicitly rather than silently inherited.
- **`d.networks`'s re-resolution target list grows unbounded across process uptime** (pre-mortem.md #4, P2): every IP ever seen gets re-resolved every cycle forever. Accepted for now because in practice this tool's IP diet is small and stable (a handful of networks total, not hundreds), and Epic 4.1's per-cycle `duration` log field gives an observable early-warning signal (creeping cycle duration) if this ever becomes real — an eviction policy can be added later, triggered by that signal, rather than speculatively now.
- **`tailscale.com/net/netmon`'s go.sum diff check (Story 3.1.1) is self-graded**, with no second reviewer or objective "alarming" threshold beyond the implementer's own judgment. Accepted as proportionate for a single-developer tool with no second engineer available to review it.
...
- Define `HostnameDetector` struct with fields: `srv *server.Server`, `detectFn func(context.Context) []string` (defaults to a context-aware wrapper around `detectLANIPs`), `resolveFn func(context.Context, string) []string` (defaults to a context-aware wrapper around `resolveLANHostnames`), `cycleTimeout time.Duration` (defaults to `30 * time.Second` — see Task 2.1.1b-timeout / pre-mortem.md P1), `networks map[string][]string`, `tick <-chan time.Time`, `events <-chan struct{}`, `manual chan chan redetectCycle` (unbuffered; the sole channel through which any caller outside `Run`'s own goroutine — e.g. the manual-trigger HTTP handler in Task 4.2.2a — may request a cycle, so every mutation of `d.networks` stays serialized inside `Run`'s single `select` loop), `done chan struct{}`.
...
- `func (d *HostnameDetector) redetect(ctx context.Context, trigger TriggerSource) redetectCycle`: snapshot `time.Now()`; derive a bounded per-cycle context via `cycleCtx, cancel := context.WithTimeout(ctx, d.cycleTimeout)` (default `d.cycleTimeout = 30 * time.Second`, well under the 5-minute interval — see pre-mortem.md #1: a hung `scutil`/`avahi-resolve`/`hostname` subprocess call, exactly the kind that's likeliest during an unstable network transition, must not freeze `Run`'s entire single-goroutine `select` loop — timer, OS-event, and manual-trigger paths all go dark together if one call never returns); `defer cancel()`; call `d.detectFn(cycleCtx)` for LAN IPs; for **every** currently-detected IP (not just newly-seen ones — see Story 2.1.1's acceptance criteria and Task 2.2.1a), call `d.resolveFn(cycleCtx, ip)` and add-only-merge any newly-found hostnames into `d.networks[ip]`'s existing slice (dedup; "already a key" means "known, keep re-resolving," never "resolved once, skip forever" — a transient resolution failure on first sighting must not permanently freeze discovery for that IP); flatten `d.networks` into a single hostname slice; call `d.srv.SetHostnames(flattened)`; return a `redetectCycle{Trigger: trigger, Duration: time.Since(start), PrevCount: ..., NewCount: ..., Added: ...}` (logging wired in Epic 4.1). If `cycleCtx` expires mid-cycle (a call hung past `cycleTimeout`), `detectFn`/`resolveFn` return whatever partial result they have (see Task 2.1.1a's updated signatures) rather than blocking `redetect` past the bound — a timed-out cycle still returns a `redetectCycle` (with whatever partial `Added` it collected) and logs normally, so `Run`'s loop is never stuck longer than `cycleTimeout`, and the *next* tick/event naturally retries. This means one more `resolveFn` subprocess call per already-known IP per cycle than a "new IPs only" approach — intentional, and still cheap against the 5-minute default interval (dozens of calls every cycle is well within the NFR budget).
- Files: `hostname_detector.go`

##### Task 2.1.1b-timeout: Thread `context.Context` into `detectFn`/`resolveFn` and enforce per-call deadlines (~5 min) — pre-mortem.md P1
- Change `detectFn`/`resolveFn`'s signatures (Task 2.1.1a) to `detectFn func(context.Context) []string` and `resolveFn func(context.Context, string) []string`, and the real `detectLANIPs`/`resolveLANHostnames` package functions to accept and honor a `context.Context` for every `safeexec.CommandContext` call they make internally (each individual subprocess call gets `ctx` — or a `context.WithTimeout(ctx, ...)` no longer than the per-call timeouts these functions already use today — as its parent, so `cycleCtx`'s expiry propagates down to cancel an in-flight `scutil`/`avahi-resolve`/`hostname` call, not just gate the *next* call from starting).
- Add a unit test with a fake `resolveFn` that blocks until its `context.Context` is cancelled (`select { case <-ctx.Done(): return nil }`) and assert `redetect` still returns within `cycleTimeout` plus a small margin — proving a stuck dependency cannot stall `Run`'s entire loop, per pre-mortem.md failure #1's specified test.
- Files: `hostname_detector.go`, `main.go` (detectLANIPs/resolveLANHostnames signatures)
...
##### Task 3.1.2d: Manual macOS verification checklist (pre-mortem.md #5) — perform at ship time, not skipped under time pressure
- Since CI has no macOS runner (pitfalls.md §5) and Task 3.1.2c may only be able to test `Close()`'s leak-free behavior in CI (depending on whether the pinned `netmon` version exposes an injectable fake), the *real* event-firing path can only be verified by hand. Before this feature is considered done, on the primary macOS dev machine: (1) start `stapler-squad` with `STAPLER_SQUAD_HOSTNAME_REDETECT_INTERVAL` set high enough (e.g. `30m`) that a timer tick can't be mistaken for the event firing, (2) switch Wi-Fi networks (or toggle a VPN), (3) confirm a `"hostname-detect: cycle complete"` log line with `trigger=network-change` appears within a few seconds — not ~5 minutes later via the timer, which would indicate the event path silently never fired and only the timer fallback is masking it (exactly the failure mode pre-mortem.md #5 describes). Record the result (pass/fail, log excerpt) in the PR description per this repo's evidence-and-claims convention.
- Files: none (manual verification step, not code)
...
- Register `srv.Mux().HandleFunc("/api/debug/redetect-hostnames", ...)` in the `"runtime"` phase after the detector is constructed, closing over the `*HostnameDetector`. The handler:
  ...
  3. Otherwise, does **not** call `detector.redetect(...)` directly (that would race `Run`'s own goroutine over the unsynchronized `d.networks` map — see the architecture review's Blocker A). Instead it creates `respCh := make(chan redetectCycle, 1)`, sends it on `detector.manual` with a context-bound send (`select { case detector.manual <- respCh: case <-r.Context().Done(): ...; case <-time.After(10*time.Second): ... }` — the 10s timeout guards against a stalled `Run` goroutine hanging the HTTP response indefinitely), **then applies the same bound to the receive**: `select { case cycle := <-respCh: writeJSON(cycle); case <-r.Context().Done(): ...; case <-time.After(10*time.Second): ... }` (pre-mortem.md #3: guarding only the send and not the subsequent receive leaves the handler able to hang indefinitely if `Run` is mid-cycle or stuck past the send succeeding — both halves of the round trip need the same timeout).
- Files: `main.go`

##### Task 4.2.2b: Test (~4 min)
- `TestRedetectHostnamesEndpoint_LoopbackRequestTriggersCycle` (asserts the response reflects a `redetectCycle` produced by a `Run` goroutine actually consuming `detector.manual`, not a direct `redetect` call), `TestRedetectHostnamesEndpoint_NonLoopbackRequestRejected`, `TestRedetectHostnamesEndpoint_DisabledReturns503AndDoesNotSendOnManualChannel`, `TestRedetectHostnamesEndpoint_RespChReceiveTimesOutIfRunStalled` (fake detector whose `Run` goroutine never drains `d.manual`; assert the handler still returns within the bound, e.g. `504`, rather than hanging) using `httptest.NewRequest`/`httptest.NewRecorder` with a fake `HostnameDetector` (fake `detectFn`/`resolveFn`, no real subprocess calls).
