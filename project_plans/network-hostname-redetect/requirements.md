...
## Success Metrics
- After switching networks (new Wi-Fi SSID, VPN toggle, or DHCP renewal to a new subnet) **without restarting the process**, a LAN hostname newly resolvable on the new network becomes usable (served, and — per the scope decision below — trusted as a WebAuthn RPID / present in the remote-access TLS cert SAN list) **within 5 minutes via the periodic timer, or within seconds via the OS-level network-change event** (the concrete interval Phase 3 planning chose — see plan.md's "Interval choice" section — matching this doc's own re-detection-cycle language so this metric is falsifiable against a number, not left open until planning), not "after next restart."
...
**Evidence this is real, current pain (not speculative):** this project was opened because the reporter personally hit this exact failure — `netflix1.staplerhome.com` stopped resolving through `stapler-squad` after switching networks between home and a work location, requiring a manual service restart (which itself kills every live tmux session) to recover. Given this is a single-developer personal tool used across at least two regularly-alternating networks (home, work) plus occasional VPN toggles, the failure is not rare/hypothetical — it's the reporter's actual weekly-or-more experience, which is the justification for spending Medium (not Small/timer-only) appetite on also closing the "up to N minutes of staleness" gap with an OS-event trigger rather than accepting a pure-timer fallback.
