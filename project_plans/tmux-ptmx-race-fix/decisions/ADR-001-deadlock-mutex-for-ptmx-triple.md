...
`session/tmux/tmux.go` already carries 5 existing locks (`detachMutex`, `controlModeSubMu`, `controlModeStartMu`, `cmdSendMu`, `recoveryMu`). Of these, 3 (`detachMutex`, `cmdSendMu`, `recoveryMu`) are `deadlock.Mutex` (`github.com/linkdata/deadlock`, go.mod:23) — a drop-in `sync.Mutex` wrapper that detects lock-order violations and stuck-lock timeouts at runtime — and 2 (`controlModeSubMu`, `controlModeStartMu`) are plain `sync.Mutex`, both confirmed by direct grep. `deadlock.Mutex` is used 30 times *package-wide* across `session/` (not a `tmux.go`-specific figure — health.go, pty_discovery.go, instance.go, tmux/fork_metrics.go, etc.), which still supports `deadlock.Mutex` as this codebase's dominant idiom for this kind of guard, but the two counts (package-wide 30 vs. file-specific 3) should not be conflated. This repo also has a standing architectural preference, ADR-011 ("Prefer Lock-Free Concurrency Techniques"), for atomics/lock-free structures over mutexes where they fit.
...
- **Reuse `detachMutex`.** Rejected: `detachMutex` is held across one or more `ptmxMu` critical sections in *both* `Detach()` and `DetachSafely()` — each calls `closePTYAndAttachCmd()` directly (one of the three PTY-triple write sites), and `Detach()` additionally chains into `Restore()` → `RestoreWithWorkDir()` afterward, still under `detachMutex`. (An earlier version of this ADR attributed the `Restore()`/`RestoreWithWorkDir()` chain to `DetachSafely()`; it is `Detach()`, a separate method, that does this — `DetachSafely()` only calls `closePTYAndAttachCmd()` directly. The correction doesn't change the conclusion.) `deadlock.Mutex` (like `sync.Mutex`) is not reentrant, so reusing `detachMutex` itself as the PTY-triple's lock would self-deadlock the very first time either method ran.
