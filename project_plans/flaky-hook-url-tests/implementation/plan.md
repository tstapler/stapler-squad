...
|---|---|---|---|
| Poll mechanism in `server_integration_test.go` | `require.Eventually` (matches sibling `approval_handler_integration_test.go` pattern; testify already a direct dep) | Hand-rolled `for time.Now().Before(deadline) { ...; time.Sleep(x) }` loop (status quo) | This file is the *only* holdout of this pattern in the area; build-vs-buy.md's build-vs-buy analysis found zero justification to keep it — no cycle risk, no missing capability `require.Eventually` lacks for this use case. **Scope note** (raised by cross-artifact consistency review): Tasks 1.2.2/1.2.3 touch `waitForLiveInstance`/`waitForResolvedAddr`, which are *not* implicated in the reported flakiness and requirements.md's Out of Scope warns against "fixing unrelated flakiness discovered along the way." These two tasks are pure mechanism swaps — no budget, poll-interval, or behavior change, only replacing hand-rolled loops with an equivalent library call — so they don't fix or touch any flakiness, unrelated or otherwise; the out-of-scope clause therefore doesn't apply. If in doubt, defer Tasks 1.2.2/1.2.3 and ship only 1.2.1/1.2.4 (the two helpers actually on Race B's critical path) without weakening the fix. |
...
- Size: 3 min
- Detail: requirements.md's Scope → In Scope explicitly names "whether GitHub Actions runner sizing/concurrency is a contributing factor (e.g. this job sharing a small/shared runner with other concurrent jobs)" as one of the three things to evaluate, and its Rabbit Holes section directs this be answered "as far as observable (workflow YAML, concurrent job count in `build.yml`)" rather than blocked on org-level access this session doesn't have. Read `build.yml`'s job list and each job's `needs:`/`runs-on:` to confirm (a) whether the `test` job runs concurrently with other jobs in the same workflow run (e.g. `build`, `install-check`, `benchmark-gate`, `web-build-smoke` — per stack.md's research, these all only `needs: prepare`, not `needs: test`, so they DO run concurrently with `test`), and (b) whether any workflow-level `concurrency:` group serializes runs in a way relevant here. Record the finding directly in this plan's Unresolved Questions section (below) — this is an observation task, not expected to require a code change, since GH-hosted concurrent *jobs* run on separate VMs (not sharing the 4 vCPUs `test`'s own `-p 1` change addresses) and org-level runner-pool sizing is out of this item's control per Constraints.
- **Given/When/Then**: Given `build.yml`'s `test`, `build`, `install-check`, `benchmark-gate`, and `web-build-smoke` jobs all declare `needs: prepare` (not `needs: test`), when this task inspects the job graph, then it confirms these jobs run concurrently with `test` on separate GitHub-hosted VMs (not competing for `test`'s own 4 vCPUs), and records that finding — ruling out same-VM cross-job contention as a factor, while noting that GitHub's account-level concurrent-job quota (a separate, org-level resource this item cannot inspect or change) remains a theoretically possible but unverifiable residual contributor.
...
**Task 3.1.3** — Pre-register `testSocketOnce` as the presumptive cause of any post-merge recurrence (pre-mortem P1 fix).
...
- Size: 2 min
- Detail: Per `pre-mortem.md` failure mode #1 (P1): because this plan explicitly does NOT fix the already-documented `testSocketOnce` shared-tmux-socket contention, a post-merge recurrence of `TestServer_should_Write*HookURL` flakiness could be misdiagnosed as "the 60s/`-p 1` fix didn't work" rather than "a different, already-known, out-of-scope mechanism is still live" — triggering a reflexive further timeout bump the file's own comments already warn against. Add one sentence next to `waitForTmuxTeardown`'s existing comment: "If `TestServer_should_Write*HookURL` flakes again after project_plans/flaky-hook-url-tests, check whether it's this shared-tmux-socket contention before re-tuning timeouts or CI topology — file a follow-up ticket for that root cause instead of widening a budget further." Include the same sentence in the shipping PR description.
...
- All three of requirements.md's original Open Questions are otherwise resolved by research and reflected in the Pattern Decisions table:
  ...
  - "Are other jobs in `build.yml` running concurrently...contributing to runner contention?" → **Resolved by Task 2.2.1** (added after cross-artifact consistency review flagged this as a genuine in-scope coverage gap — requirements.md's Rabbit Holes section directs this be checked "as far as observable" from the workflow YAML, not that it be left uninvestigated): `build.yml`'s other jobs (`build`, `install-check`, `benchmark-gate`, `web-build-smoke`) all declare `needs: prepare`, not `needs: test`, so they run on separate GitHub-hosted VMs concurrently with `test` but do not share `test`'s 4 vCPUs — same-VM cross-job contention is ruled out. Account-level concurrent-job-quota contention (a GitHub org/billing-level resource) remains theoretically possible but is unverifiable from this repo alone and is out of this item's control per Constraints.
