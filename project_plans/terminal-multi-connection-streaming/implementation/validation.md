...
- [ ] Not yet executed. Plan.md requires this be performed against a real, disposable session on the operator's own instance before any wider rollout, and the outcome (pass/fail, date) recorded in this file. **Do not check the box above until the rehearsal has actually been run** — this is an execution record, not a design placeholder.
