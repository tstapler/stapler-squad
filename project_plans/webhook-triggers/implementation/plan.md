...
Resolves the 4 BLOCKER findings from `architecture-review.md` and `adversarial-review.md` (2026-08-06 pass). One line each; full detail inline at the referenced tasks.

- **Import-cycle / layering violation** (architecture-review.md sole Blocker; adversarial-review.md Blocker #4 — same defect, inbound-callback and chain-fire halves) — fixed via new `session.EventDispatcher`/`session.ChainFirer` consumer-owned interfaces (Domain Glossary, Pattern Decisions) + setter-injection at `server/dependencies.go`, applied to Tasks 5.2.1a/5.2.1c/5.2.1d (Epic 5.2) and Tasks 6.2.1a/6.2.1b (Epic 6.2).
- **Delivery-ID dedup race** (adversarial-review.md Blocker #1) — Task 1.2.1a now adds a genuine `.Unique()` constraint on `delivery_id`; Task 1.2.1c/2.2.1c/2.3.1b now insert-and-catch-`ent.IsConstraintError` instead of check-then-insert.
- **No SSRF validation on callback URLs** (adversarial-review.md Blocker #2) — new Task 5.1.1e (`server/services/url_validation.go`, `ValidateCallbackURL`) called at both save-time (`CallbackConfigService.UpdateCallbackConfig`) and send-time (`CallbackDispatcher.Dispatch`, Task 5.2.1a).
- **No enable/disable field for non-cron triggers** (adversarial-review.md Blocker #3; overlaps architecture-review.md's FR6/AC7 concern) — new generic `trigger_enabled` field added in Task 1.1.1a, backfilled from `cron_enabled` in Task 1.1.1d, checked in Tasks 2.2.1b/2.3.1a (webhook handlers) and Task 3.2.1a (`FireTrigger`, covers cron); Story 7.1.1's previously-open "or a new generic Enabled field" bracket is now resolved.
...
| `TriggerFireOutcome` | Enum-shaped string on `TriggerFireEvent`: `"fired_success"` \| `"fired_failed"` \| `"no_match"` \| `"rejected"`. | Five UX states (research/ux.md §4) collapse to these four backend outcomes + `"disabled"` (derived from `Workflow.TriggerEnabled`, not stored per-event — see `TriggerEnabled` below). A disabled workflow reached via `FireTrigger` (e.g. a stale cron tick registered before a live disable) persists `Outcome: "fired_failed"` with `ErrorMessage: "trigger disabled"` — it was a genuine fire attempt, unlike a webhook handler simply excluding a disabled workflow from its candidate set (Task 2.2.1b/2.3.1a), which falls through to the existing `"no_match"` path with no enum change needed. |
| `TriggerEnabled` | New `field.Bool("trigger_enabled").Default(true)` on `ent.Workflow` — generic enable/disable for **all** trigger types (FR6/AC7), not just cron. | Added during review repair (adversarial-review.md Blocker #3): the pre-existing `cron_enabled` only gated cron; `github_push`/`webhook` triggers had no equivalent field. Backfilled from `CronEnabled`'s current value for existing rows (Task 1.1.1d) rather than a true field rename, to stay consistent with this plan's additive-only ent migration convention (see Migration Plan). `cron_enabled` remains on the schema for backward-compat display but is no longer read by any new gating logic — `TriggerEnabled` is the single source of truth checked by both webhook handlers and `FireTrigger`. |
| `DeliveryID` | Dedup key: GitHub's `X-GitHub-Delivery` header value, or a computed SHA-256 digest of the raw body for generic webhooks with no provider-assigned ID. Enforced via a genuine `.Unique()` ent constraint on `TriggerFireEvent.delivery_id` (Task 1.2.1a), not just a lookup index. | Dedup is insert-and-catch-`ent.IsConstraintError`, not check-then-insert (adversarial-review.md Blocker #1 — see Task 1.2.1c) — closes the race a plain existence-check-before-insert leaves open under concurrent/replayed deliveries. |
...
| `FireTrigger` | New `Scheduler` method (`server/workflows/scheduler.go`) generalizing `FireNow` to accept an already-rendered prompt + `TriggerType` + `DeliveryID`, used by both cron ticks and inbound webhook handlers. | Cron's own fire path becomes a thin caller of `FireTrigger`. Also the single place that satisfies `session.ChainFirer` (see below) and gates on `TriggerEnabled`. |
| `EventDispatcher` | New narrow interface **defined in the `session` package** (`session/event_dispatcher.go`): `Dispatch(eventType string, payload any)`. Satisfied structurally by `server/services.CallbackDispatcher` (no explicit `implements` needed — Go interfaces are implicit). | Added during review repair (architecture-review.md's sole Blocker / adversarial-review.md Blocker #4): `session/ent_repository_backlog.go` and `session/backlog_lifecycle.go` are package `session`, which `server/services` and `server/workflows` already import — a direct call from `session` into `server/services.CallbackDispatcher` would be a compile-breaking import cycle. Consumer-owned interface, injected via setter at `server/dependencies.go`, mirrors the `AdmissionGate` pattern Epic 1.3 already established. |
| `ChainFirer` | New narrow interface **defined in the `session` package** (`session/chain_firer.go`): `FireTrigger(ctx context.Context, wf *ent.Workflow, renderedPrompt string, deliveryID string) (string, error)`. Satisfied structurally by `server/workflows.Scheduler`. | Same import-cycle rationale and injection pattern as `EventDispatcher`, for the chain-fire call in Epic 6.2 (adversarial-review.md Blocker #4's other half). |
...
| `CallbackConfig` | New nested config struct (`config/types.go`), embedded on `config.Config` as `Callbacks CallbackConfig`: `OnSessionCompleteURL`, `OnSessionStaleURL`, `OnQueueItemCreatedURL`. | Mirrors `SlackConfig`'s placement (`config/config.go`). Global singleton URLs (FR7's literal "each accept a URL," singular). Every write path validates via `ValidateCallbackURL(url string) error` (new, `server/services/url_validation.go`, Task 5.1.1e) — scheme allowlist (`http`/`https`) + resolve-and-reject loopback/link-local/RFC1918/cloud-metadata (`169.254.169.254`) targets. |
| `CallbackDispatcher` | New concrete type (`server/services/callback_dispatcher.go`): async, bounded-retry (3 attempts), independent `context.WithTimeout(5s)` JSON POST dispatcher. Satisfies `session.EventDispatcher` (above). | Directly generalizes `SlackNotifier` from `project_plans/slack-review-notifications`. Every `Dispatch` attempt calls `ValidateCallbackURL` (see `CallbackConfig` row below) immediately before each POST — send-time, not just save-time, since DNS can change between config-save and a later fire. |
...
| Cross-aggregate calls from `TransitionBacklogItemStatus`/reconciler code (callback dispatch, chain-fire) | Narrow consumer-owned interfaces defined **in `session`** (`session.EventDispatcher`, `session.ChainFirer`), satisfied structurally by `server/services.CallbackDispatcher`/`server/workflows.Scheduler`, injected via setter at `server/dependencies.go` | `AdmissionGate` (Epic 1.3, Task 1.3.1a/b) — the plan's own precedent for exactly this problem | Direct call from `session/ent_repository_backlog.go`/`session/backlog_lifecycle.go` into `server/services`/`server/workflows` types (the plan's original shape for Tasks 5.2.1c/6.2.1a/6.2.1b) | `server/services` and `server/workflows` already import `session` — a direct call the other way is a compile-breaking import cycle, confirmed via `grep` (architecture-review.md's sole Blocker; adversarial-review.md Blocker #4). Also restores Repository/Clean-Architecture inward-only dependency direction: the persistence adapter depends on an interface it owns, not on an application-layer collaborator. |
| Delivery-ID dedup | DB-level `.Unique()` constraint on `TriggerFireEvent.delivery_id`; insert-and-catch-`ent.IsConstraintError` (same idiom as `session/ent_workflow_repository.go`'s slug-uniqueness handling) | `ent_pipeline_mode_repository.go`/`ent_workflow_repository.go`'s existing `ent.IsConstraintError` pattern for unique-slug conflicts | Read-then-write: `ExistsByDeliveryID` check followed by a separate insert (the plan's original shape) | Check-then-insert races under concurrent/replayed deliveries — two requests can both pass the existence check before either writes, double-firing (adversarial-review.md Blocker #1). A DB constraint makes the race structurally impossible instead of narrowing the window. |
| Outbound callback URL SSRF guard | `ValidateCallbackURL(url string) error` (`server/services/url_validation.go`), scheme allowlist + resolve-and-reject private/loopback/link-local/metadata targets, called at both save-time (`CallbackConfigService.UpdateCallbackConfig`) and send-time (`CallbackDispatcher.Dispatch`, every attempt) | pitfalls research §5's explicit requirement | Save-time-only validation | DNS can change between config-save and a later callback fire (TOCTOU) — pitfalls §5 explicitly calls for send-time validation, not just save-time; validating both closes the gap adversarial-review.md Blocker #2 identified (zero validation existed at either point in the original plan) |
| Trigger enable/disable | Single generic `trigger_enabled` bool field on `ent.Workflow`, checked by both webhook handlers (candidate filtering) and `FireTrigger` (cron + defense-in-depth) | FR6/AC7; `cron_enabled`'s existing shape generalized | Leaving only the pre-existing cron-specific `cron_enabled` field, letting Phase 7's UI guess at a "new generic Enabled field" per-trigger-type | `github_push`/`webhook` triggers had no enable/disable field at all — a "disabled" trigger of either type had nothing to check and would keep firing (adversarial-review.md Blocker #3); one field checked at one shared choke point (`FireTrigger`) plus early filtering in the handlers (to skip wasted decrypt/verify work) is simpler than a per-type field |
...
- **Reversibility**: All new `Workflow`/`BacklogItem` columns are `.Optional()` with safe zero-value defaults (`trigger_type` defaults to `"manual"`/`"cron"` per the backfill rule below; `trigger_enabled` defaults `true`; `chain_fired` defaults `false`; `webhook_slug`/`webhook_secret_encrypted`/`next_workflow_id` default empty/nil). `TriggerFireEvent.delivery_id` carries a genuine `.Unique()` constraint (not just an index) — additive and safe, since the table is new and starts empty. Rolling back the binary to a pre-feature version leaves these columns present but unread — no destructive down-migration needed. The new `trigger_fire_event` table is additive and safe to leave orphaned on rollback.
- **Zero-downtime strategy**: Same as every other ent-schema change in this codebase — ent's additive auto-migration runs on next boot before the HTTP server starts accepting the new routes; existing `Workflow` rows (all currently `cron`-only, since that's the only trigger type that exists today) are backfilled with `trigger_type = "cron"` if `cron_enabled = true` else `"manual"`, **and** `trigger_enabled = cron_enabled`'s current value (preserving existing on/off state 1:1 — no behavior change for existing cron-workflow users) in a one-time data-backfill step (Task 1.1.1d) that runs inside the same migration transaction/boot sequence, so no window exists where a row has `trigger_type = ""`.
...
##### Task 1.1.1a: Add trigger fields to `session/ent/schema/workflow.go` (~5 min)
- Add `field.String("trigger_type").Optional().Default("manual")`, `field.String("github_repo").Optional()`, `field.String("github_branch").Optional()`, `field.String("webhook_slug").Optional().Unique()`, `field.String("webhook_secret_encrypted").Optional()`, `field.String("event_filter").Optional()`, `field.String("label_filter").Optional()`, `field.String("prompt_template").Optional()`, `field.Time("last_fired_at").Optional().Nillable()`, `field.Bool("trigger_enabled").Default(true)` (generic enable/disable for all trigger types — see `TriggerEnabled` in Domain Glossary; resolves adversarial-review.md Blocker #3, which found `github_push`/`webhook` triggers had no enable/disable field at all).
...
##### Task 1.1.1d: One-time backfill of `trigger_type` and `trigger_enabled` on existing rows (~4 min)
- On `Scheduler.Start` (or a dedicated boot-time migration function called once before `Start`), for any `Workflow` row with `trigger_type == ""`, set `trigger_type = "cron"` if `CronEnabled` else `"manual"`, **and** set `trigger_enabled = CronEnabled`'s current value (so existing cron-workflow on/off state carries over unchanged — `trigger_enabled` becomes the one field all subsequent gating logic reads, `cron_enabled` is left in place unread by new code), persisted via `repo.Update`.
...
##### Task 1.2.1a: Create `session/ent/schema/trigger_fire_event.go` (~5 min)
- Model on `session/ent/schema/source_sync_event.go`: `field.UUID("id")`, `field.UUID("workflow_id").Optional().Nillable()` (nil when slug unknown — rejected before a `Workflow` is resolved), `field.String("outcome").NotEmpty()`, `field.String("delivery_id").Optional().Unique()` (**genuine DB-level uniqueness, not just an index** — resolves adversarial-review.md Blocker #1: a plain lookup index leaves a check-then-insert race where two near-simultaneous requests with the same delivery ID can both pass an existence check before either writes; `.Unique()` makes the DB reject the second insert outright. `Optional()` is required because generic-webhook dedup uses a SHA-256 body digest that is always present, but the field stays optional at the ent level for rows created before a delivery ID is known), `field.String("session_id").Optional()`, `field.String("error_message").Optional()`, `field.Time("created_at").Default(time.Now).Immutable()`.
- Add `index.Fields("workflow_id")`, `index.Fields("created_at")` (drop the separate `index.Fields("delivery_id")` — `.Unique()` already creates the backing index).
...
##### Task 1.2.1c: Repository methods (~5 min)
- New `session/trigger_fire_event_repository.go`: `Create(ctx, TriggerFireEventInput) error`, `ListByWorkflow(ctx, workflowID uuid.UUID, limit int) ([]*ent.TriggerFireEvent, error)`.
- Add `ClaimDeliveryID(ctx, deliveryID string, workflowID uuid.UUID) (claimed bool, err error)`: attempts an insert of a placeholder/in-progress `TriggerFireEvent` row carrying `delivery_id`; on `ent.IsConstraintError(err)` (same idiom as `session/ent_workflow_repository.go`'s slug-uniqueness handling), returns `(false, nil)` — "already processed, not a real error." **No separate `ExistsByDeliveryID` read-then-write** — the insert attempt *is* the dedup check (adversarial-review.md Blocker #1: a pre-check followed by a later insert leaves a window where two near-simultaneous requests both pass the check before either writes). Both webhook handlers (Tasks 2.2.1c/2.3.1b) call `ClaimDeliveryID` and treat `claimed == false` exactly as they previously treated `ExistsByDeliveryID == true`.
...
- Candidate set = `github_push` `Workflow`s matching the push's repo **and** `TriggerEnabled == true` — filter disabled workflows out here, before any decrypt/verify work (adversarial-review.md Blocker #3: closes the "disabled trigger keeps firing" gap; also avoids wasted decrypt cycles on a workflow that can never fire). Decrypt each remaining candidate's `WebhookSecretEncrypted` and call `VerifyGitHubSignature` against the raw bytes (not re-marshaled JSON, per pitfalls §1.1).
- Files: `server/services/github_webhook_handler.go`.

##### Task 2.2.1c: Delivery-ID dedup via atomic claim (before any session-creation work) (~4 min)
- Read `X-GitHub-Delivery` header; call `fireEvents.ClaimDeliveryID(ctx, deliveryID, wf.ID)` (Task 1.2.1c) — if `claimed == false`, return 200 immediately ("already processed") without re-firing (pitfalls §2.2). Unlike a plain existence check, this is race-safe under concurrent/replayed deliveries because the claim attempt and the uniqueness check are the same DB operation (adversarial-review.md Blocker #1).
- Files: `server/services/github_webhook_handler.go`.
...
##### Task 2.3.1a: Scaffold `GenericWebhookHandler` + `{slug}` route (~5 min)
...
- If the slug resolves but `!wf.TriggerEnabled`: return HTTP 200, persist `TriggerFireEvent{Outcome: "no_match", ErrorMessage: "trigger disabled"}`, and stop **before** secret verification/dedup/matching (adversarial-review.md Blocker #3 — a disabled slug does no further work, same short-circuit intent as an unmatched candidate in the GitHub handler, Task 2.2.1b).
- Files: `server/services/generic_webhook_handler.go`.

##### Task 2.3.1b: Body-size cap, JSON parse, secret verification, delivery-ID dedup via atomic claim (~5 min)
- `http.MaxBytesReader`, `json.Unmarshal` into `map[string]interface{}` (400 on parse error), `VerifyWebhookSecret` against raw bytes (401 on mismatch), delivery-ID = SHA-256 digest of raw body (no provider-assigned ID for generic webhooks — pitfalls §1.4) claimed via `fireEvents.ClaimDeliveryID(ctx, deliveryID, wf.ID)` (Task 1.2.1c) — `claimed == false` means already processed, race-safe under concurrent replays (adversarial-review.md Blocker #1).
- Files: `server/services/generic_webhook_handler.go`.
...
#### Story 2.4.1: Delivery-ID dedup is enforced via a DB-level unique constraint on the durable `TriggerFireEvent` table
**Acceptance Criteria**:
- A replayed GitHub delivery (same `X-GitHub-Delivery`) does not create a second session, even when the replay arrives concurrently with the original (not just after it completes).
  - *Given* a `TriggerFireEvent{DeliveryID: "abc-123", Outcome: "fired_success"}` already persisted, *When* a second `POST /webhooks/github` arrives with `X-GitHub-Delivery: abc-123`, *Then* `ClaimDeliveryID` returns `(false, nil)` (constraint violation caught), the handler returns 200 immediately, and no second `CreateSession` call occurs.
  - *Given* two requests with the same `X-GitHub-Delivery: abc-123` arrive **simultaneously** (both pass any in-memory check before either has written), *When* both call `ClaimDeliveryID`, *Then* exactly one succeeds (`claimed == true`) and the other's insert fails with `ent.IsConstraintError`, closing the race adversarial-review.md Blocker #1 identified in the original check-then-insert shape.
**Files**: covered by Tasks 1.2.1a (`.Unique()` constraint), 1.2.1c (`ClaimDeliveryID`), 2.2.1c, 2.3.1b (already implemented above) — this story is the cross-cutting AC verification, not new code.

##### Task 2.4.1a: Integration test proving dedup across both handler types, including a concurrent-arrival case (~5 min)
- One test per handler asserting `SessionService.CreateSession` call count stays at 1 after two identical (same delivery-ID) requests. Add a third test firing both requests concurrently (goroutines + a `sync.WaitGroup`) against the same `delivery_id` to exercise the constraint-violation path directly, not just the sequential "already exists" path.
...
##### Task 3.2.1a: Add `FireTrigger(ctx, wf *ent.Workflow, renderedPrompt string, deliveryID string) (string, error)` (~6 min)
- **First line of the method**: `if !wf.TriggerEnabled { persist TriggerFireEvent{Outcome: "fired_failed", ErrorMessage: "trigger disabled"}; return "", ErrTriggerDisabled }` — this is the single choke point both cron ticks and (redundantly, defense-in-depth) both webhook handlers pass through, so it's the one place that must not be skippable (adversarial-review.md Blocker #3). Covers the cron path directly: even if `Scheduler.Reload`'s hot-swap hasn't yet deregistered a just-disabled workflow's `cron.AddFunc` tick, a fire attempt that reaches `FireTrigger` is still rejected here.
...
- This method is also where `session.ChainFirer` is satisfied (Epic 6.2, Domain Glossary) — no signature change needed, `Scheduler` already has the right shape.
- Files: `server/workflows/scheduler.go`.
...
- For each `Workflow` loaded on `Start` with `trigger_type == "cron"` **and** `TriggerEnabled == true` (a disabled cron workflow has no "missed fire" to report), use the cron parser's `Schedule.Next(t)` to compute the expected previous occurrence before "now"; if `wf.LastFiredAt` is nil or older than that occurrence (and the workflow has existed longer than one cron period — avoid false-positive on brand-new workflows), log a missed-fire warning.
- Files: `server/workflows/scheduler.go`.
...
**Files**: `config/types.go`, `config/config.go`, `proto/session/v1/session.proto`, `server/services/callback_config_service.go` (new), `server/services/url_validation.go` (new — SSRF guard, Task 5.1.1e).
...
##### Task 5.1.1e: `ValidateCallbackURL` — save-time SSRF guard (~5 min)
- New `func ValidateCallbackURL(rawURL string) error` in `server/services/url_validation.go`: parse with `net/url`, reject any scheme other than `http`/`https`; resolve the host via `net.LookupIP` (or `net.Resolver.LookupIPAddr` for context support) and reject if any resolved address is loopback (`IsLoopback`), link-local (`IsLinkLocalUnicast`/`IsLinkLocalMulticast`), a private RFC1918 range (`IsPrivate`), or the cloud-metadata address `169.254.169.254` specifically (covered by `IsLinkLocalUnicast` but called out explicitly since it's the highest-value SSRF target per pitfalls §5).
- Call it from `CallbackConfigService.UpdateCallbackConfig` (Task 5.1.1d) for each of the three URL fields that's being set/changed; reject with `connect.CodeInvalidArgument` on failure — this is the **save-time** half of adversarial-review.md Blocker #2's fix (the **send-time** half, required because DNS can change after save, is Task 5.2.1a).
- Files: `server/services/url_validation.go` (new), `server/services/callback_config_service.go`.
...
**Layering note (review repair)**: `TransitionBacklogItemStatus` lives in `session/ent_repository_backlog.go`, and `on_session_stale`'s hook point (`reconcileStaleWorkSessions`) lives in `session/backlog_lifecycle.go` — both package `session`. `server/services` (where `CallbackDispatcher` lives) already imports `session`, so a direct call the other way is a compile-breaking import cycle (architecture-review.md's sole Blocker; adversarial-review.md Blocker #4). Fix chosen: a narrow `session.EventDispatcher` interface (`Dispatch(eventType string, payload any)`), defined in `session/event_dispatcher.go`, satisfied structurally by `CallbackDispatcher`, injected via setter at `server/dependencies.go` — same pattern as `AdmissionGate` (Epic 1.3). (`ReactiveQueueManager.OnItemAdded`, Task 5.2.1b, is package `server`, not `session` — it can call `CallbackDispatcher` directly with no cycle, and is unaffected by this fix.)
**Acceptance Criteria**:
- AC4: On session completion, a configured `on_session_complete` URL receives a POST with session outcome data; delivery failure does not block or corrupt the session's own state transition.
  ...
  - *Given* `cfg.Callbacks.OnSessionCompleteURL = "http://169.254.169.254/latest/meta-data/"` (a cloud-metadata SSRF target that somehow reached config, e.g. via direct DB edit bypassing the save-time guard), *When* `CallbackDispatcher.Dispatch` attempts delivery, *Then* `ValidateCallbackURL` rejects it before any HTTP request is made, and the rejection is logged without ever issuing the POST (adversarial-review.md Blocker #2's send-time half).
**Files**: `server/services/callback_dispatcher.go`, `server/review_queue_manager.go`, `session/ent_repository_backlog.go` (or wherever `TransitionBacklogItemStatus`'s done-transition hook belongs), `session/backlog_lifecycle.go`, `session/event_dispatcher.go` (new — interface), `server/dependencies.go` (setter wiring).

##### Task 5.2.1a: Define `session.EventDispatcher` interface, implement `CallbackDispatcher` (incl. send-time SSRF guard), wire via setter injection (~8 min)
- New `session/event_dispatcher.go`: `type EventDispatcher interface { Dispatch(eventType string, payload any) }` — defined in the **consumer** package (`session`), same rationale as `AdmissionGate` (Epic 1.3, `.claude/rules/interface-pollution-checklist.md`).
- `type CallbackDispatcher struct { client *http.Client; cfg *config.Config; sem chan struct{} }` — **(Pre-mortem P1 fix)** `sem` is a bounded semaphore (e.g. `make(chan struct{}, 16)`) guarding concurrent in-flight dispatch goroutines, closing the unbounded-goroutine risk adversarial-review.md's Concerns section flagged and this repo's 2026-07-12 OOM incident makes non-hypothetical. `func (d *CallbackDispatcher) Dispatch(eventType string, payload any)`: attempt a non-blocking acquire (`select { case d.sem <- struct{}{}: ... default: log.Warn("[CallbackDispatcher] dispatch queue full, dropping", "event", eventType); return }` — a full semaphore drops-and-logs rather than blocking the caller or growing unbounded, satisfying FR9's "not silently dropped" for this path too); on acquire, `go func() { defer func() { <-d.sem }(); url := d.urlFor(eventType); if err := ValidateCallbackURL(url); err != nil { log.Warn("[CallbackDispatcher] rejected callback URL", "event", eventType, "reason", err); return }; for attempt := 0; attempt < 3; attempt++ { ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second); ... POST ...; cancel(); if success { return }; time.Sleep(backoff) }; log.Warn("[CallbackDispatcher] delivery failed after retries", "event", eventType) /* URL never logged */ }()`. `ValidateCallbackURL` (Task 5.1.1e) runs on **every** dispatch attempt, not just once at startup — this is the send-time half of adversarial-review.md Blocker #2 (DNS can change between config-save and a later fire, so a save-time-only check is insufficient). `CallbackDispatcher` satisfies `session.EventDispatcher` structurally — no explicit declaration needed.
- In `server/dependencies.go`, after `CallbackDispatcher`, the `EntRepository`/`Storage`, and `BacklogLifecycleListener` are all constructed, wire the interface with setters (mirrors Task 1.3.1b's `SetAdmissionGate` — avoids a breaking constructor change to either `session` type): `entRepository.SetEventDispatcher(callbackDispatcher)` and `backlogLifecycleListener.SetEventDispatcher(callbackDispatcher)`. Both `EntRepository` and `BacklogLifecycleListener` gain a nilable `eventDispatcher EventDispatcher` field, guarded (`if r.eventDispatcher != nil`) at each call site so the feature flag / not-yet-wired case is a safe no-op.
- Files: `server/services/callback_dispatcher.go`, `session/event_dispatcher.go` (new), `server/dependencies.go`.

##### Task 5.2.1b: Wire `on_queue_item_created` at `ReactiveQueueManager.OnItemAdded` (~3 min)
- Add `if rqm.cfg.Callbacks.OnQueueItemCreatedURL != "" { rqm.callbackDispatcher.Dispatch("queue_item_created", payload) }` next to the existing `rqm.eventBus.Publish(...)` call (`server/review_queue_manager.go` ~line 411) — **pending resolution of the Unresolved Question about whether this is the FR7-intended event**. `server/review_queue_manager.go` is package `server`, which already imports `server/services` directly — no interface indirection needed here, unlike Tasks 5.2.1c/5.2.1d below.
- Files: `server/review_queue_manager.go`.

##### Task 5.2.1c: Wire `on_session_complete` at the `BacklogStatusDone` transition (~4 min)
- Inside `EntRepository.TransitionBacklogItemStatus` (`session/ent_repository_backlog.go:918`), after a successful CAS to `BacklogStatusDone`, call `r.eventDispatcher.Dispatch("session_complete", payload)` through the injected `session.EventDispatcher` field (Task 5.2.1a) — **not** a direct call to `server/services.CallbackDispatcher`, which would create the import cycle architecture-review.md's sole Blocker / adversarial-review.md Blocker #4 identified.
...
- After `er.MarkStuck(ctx, item.ID, domain.StuckReasonStaleWork, ...)` succeeds (first crossing only — reuse whatever dedup `MarkStuck` already provides, per architecture.md's "only fire once per crossing" requirement), dispatch `on_session_stale` via `l.eventDispatcher.Dispatch(...)` — same injected `session.EventDispatcher` field (Task 5.2.1a), same cycle rationale as Task 5.2.1c (`session/backlog_lifecycle.go` is also package `session`).
- Files: `session/backlog_lifecycle.go`.

##### Task 5.2.1e: Tests — non-blocking dispatch, redaction, bounded retry, SSRF rejection (~6 min)
- `httptest.NewServer` that hangs/never responds; assert the calling transition returns well before the dispatcher's 5s timeout elapses. Separate test asserting a URL with embedded credentials never appears in a captured log line. New test: a callback URL resolving to `127.0.0.1`/`169.254.169.254`/an RFC1918 address is rejected by `ValidateCallbackURL` inside `Dispatch` before any HTTP request is attempted (assert via a request-counting `http.RoundTripper` that sees zero calls).
...
**Layering note (review repair)**: adversarial-review.md Blocker #4 flagged that `session/ent_repository_backlog.go` and `session/backlog_lifecycle.go` (package `session`) calling `server/workflows.Scheduler.FireTrigger` directly is a compile-breaking import cycle (`server/workflows` already imports `session`). Same fix as Epic 5.2: a narrow `session.ChainFirer` interface (`FireTrigger(ctx, wf *ent.Workflow, renderedPrompt, deliveryID string) (string, error)`), defined in `session/chain_firer.go`, satisfied structurally by `server/workflows.Scheduler` (Task 3.2.1a already gives it the right method shape — no signature change needed there), injected via setter at `server/dependencies.go`.
**Acceptance Criteria**:
...
**Files**: `session/ent_repository_backlog.go`, `session/backlog_lifecycle.go`, `server/workflows/scheduler.go`, `session/chain_firer.go` (new — interface), `server/dependencies.go` (setter wiring).

##### Task 6.2.1a0: Define `session.ChainFirer` interface + wire via setter injection (~4 min)
- New `session/chain_firer.go`: `type ChainFirer interface { FireTrigger(ctx context.Context, wf *ent.Workflow, renderedPrompt string, deliveryID string) (string, error) }` — consumer-owned, same rationale/shape as `session.EventDispatcher` (Task 5.2.1a) and `AdmissionGate` (Epic 1.3).
- In `server/dependencies.go`, after `Scheduler` and the `EntRepository`/`BacklogLifecycleListener` are constructed: `entRepository.SetChainFirer(workflowScheduler)`, `backlogLifecycleListener.SetChainFirer(workflowScheduler)` (setter injection, same pattern as `SetEventDispatcher`/`SetAdmissionGate` — avoids breaking either constructor). Both types gain a nilable `chainFirer ChainFirer` field.
- Files: `session/chain_firer.go` (new), `server/dependencies.go`.

##### Task 6.2.1a: Chain-fire attempt inside `TransitionBacklogItemStatus`'s done branch (~5 min)
- **(Pre-mortem P1 fix)** After the CAS succeeds and `NextWorkflowID != nil`, and only *after* the transition's own DB transaction has committed and released (do not hold a lock/transaction open across `FireTrigger`'s `CreateSession` call — a tmux+git-worktree spin-up is too slow to run inside a persistence-layer transaction), attempt the chain fire via `go func() { r.chainFirer.FireTrigger(...) }()` (injected `session.ChainFirer`, Task 6.2.1a0), mirroring `EventDispatcher`'s async-dispatch pattern (Task 5.2.1a) rather than blocking the caller — **not** a direct call to `server/workflows.Scheduler`, which would create the import cycle adversarial-review.md Blocker #4 identified. On success set `ChainFired = true` in a follow-up update; on any error (including `chainFirer == nil`, i.e. not wired / feature-flagged off) log at `log.Warn` distinguishing "not wired" from "fire failed" (closes pre-mortem P2 #4's silent-no-op gap) and leave `ChainFired = false` (the reconciler will retry). Add a test proving no DB lock/transaction is held during a deliberately slow `FireTrigger` call.
- Files: `session/ent_repository_backlog.go`.

##### Task 6.2.1b: `TriggerChainReconciler.ReconcileChains` on the existing 60s ticker (~5 min)
- Modeled on `reconcileStaleWorkSessions` (`session/backlog_lifecycle.go:2294`): `ListBacklogItems(ctx, BacklogItemFilter{Statuses: [done]})`, filter `NextWorkflowID != nil && !ChainFired`, attempt chain-fire for each via the same injected `session.ChainFirer` field (`l.chainFirer.FireTrigger(...)`), set `ChainFired = true` on success.
- Files: `session/backlog_lifecycle.go`.
...
- AC7: Trigger and callback configuration can be added/edited/disabled without restarting the service.
  - *Given* a `Workflow{TriggerType: "webhook", TriggerEnabled: true}` row shown in `TriggersPanel`, *When* a user clicks its toggle, *Then* `UpdateWorkflow{TriggerEnabled: false}` fires immediately (the generic field resolved in the Review Repair Log — see `TriggerEnabled` in the Domain Glossary, not a per-type field), the row grays out (`rowDisabled` class), and no service restart occurs.
