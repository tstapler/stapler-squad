...
##### Task 3.3.3a: Reuse `services.HasActiveReviewSession` for message branching (~4 min)
- After the transition + notes persistence, call `itemSessions, _ := h.storage.ListItemSessions(ctx, itemID)` and `services.HasActiveReviewSession(itemSessions)` (the same exported helper Task 2.2.1a wires up — no duplication) to pick between the two message templates above.
- Files: `server/mcp/tools_backlog.go`
