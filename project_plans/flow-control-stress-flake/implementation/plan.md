...
**Correction (post-review, round 2)**: the per-iteration `await Promise.all([writeDone, jest.advanceTimersByTimeAsync(1)])` race pattern above was implemented and passed isolated + synthetic-contention verification, but a review reproduced the *exact* full-suite scenario from the original bug report (`npx jest --no-coverage`, no isolation) and got 2/3 failures — `jest.advanceTimersByTimeAsync` inserts a real microtask-queue flush per fired timer, and 5000 of those per test still cost real wall-clock time that compounds under contention, which is exactly what AC1 rules out. Measured directly (see `zzz-bench` throwaway harness, not committed): the async variant alone took ~12.9s isolated for 5000 timers, and batching all 5000 into one `advanceTimersByTimeAsync` call made it *worse* (>30s / effectively hung) — the per-timer async overhead doesn't amortize by calling it once. Switched to the **synchronous** `jest.advanceTimersByTime()`, called once after the whole loop runs synchronously (no per-iteration await at all): none of `WatermarkTracker.write()`'s callbacks are themselves async, so nothing requires the async variant's microtask-flush guarantee. Measured result: 5000-timer flush dropped to ~2.6s isolated, ~7.3s under 24-way synthetic CPU contention (`yes > /dev/null` × nproc), and passed 3/3 direct re-runs of the full `npx jest --no-coverage` suite (the reviewer's own repro condition).
