...
// via buildScrollbackResponse. Widened by Task 1.3.3c to also route an
// app-forwarded scroll result to AppScrollbackResponse, mirroring
// handleScrollbackRequest's branch -- shell tabs have no *streamhub.StreamHub
// of their own (streamShellViaControlMode is always control-mode-direct), so
// this always passes hub: nil and the same -1 subscriberCount sentinel
// streamViaControlMode's closure uses, per Risk Control's "Concrete
// resolution" section: AppScrollGate's check (4) always fails here, so this
// call site never actually forwards a scroll -- the widening exists so a
// future adapter/path change doesn't have to rediscover this plumbing.
