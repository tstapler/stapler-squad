...
func TestReportPRCreated_should_TransitionToPRPending_When_ValidPR(t *testing.T) {
	...
	handler := &backlogHandlers{
		...
		verifyPRMatchesBranch: func(context.Context, string, string, int, string) (PRVerification, error) {
			return NewPRVerification(true, true, "backlog/ship-it", github.PRStateOpen, "tstapler"), nil
		},
	}
	ctxWithUUID := WithSessionUUID(context.Background(), sessionUUID)
...
func TestReportPRCreated_should_RejectCall_When_BranchMismatch(t *testing.T) {
	...
	handler := &backlogHandlers{
		...
		verifyPRMatchesBranch: func(_ context.Context, _, _ string, _ int, expectedBranch string) (PRVerification, error) {
			...
			return NewPRVerification(true, false, "totally-unrelated-branch", github.PRStateOpen, "tstapler"), nil
		},
	}
	ctxWithUUID := WithSessionUUID(context.Background(), sessionUUID)
...
func TestDecideOverridePolicy(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			v:              NewPRVerification(true, true, "backlog/x", github.PRStateOpen, "tstapler"),
			overrideReason: "",
			callerLogin:    "",
			wantAccept:     true,
		},
		{
			...
			v:              NewPRVerification(true, false, "feature/y", github.PRStateOpen, "tstapler"),
			overrideReason: "",
			callerLogin:    "tstapler",
			wantAccept:     false,
		...
		},
		{
			...
			v:              NewPRVerification(true, false, "feature/y", github.PRStateOpen, "someone-else"),
			overrideReason: "here's why",
			callerLogin:    "tstapler",
			wantAccept:     false,
		...
		},
		{
			...
			v:              NewPRVerification(true, false, "feature/y", github.PRStateClosed, "someone-else"),
			overrideReason: "here's why",
			callerLogin:    "tstapler",
			wantAccept:     false,
			wantCode:       connect.CodePermissionDenied,
		},
		{
			...
			v:              NewPRVerification(true, false, "feature/y", github.PRStateClosed, "tstapler"),
			overrideReason: "here's why",
			callerLogin:    "tstapler",
			wantAccept:     false,
		...
		},
		{
			...
			v:              NewPRVerification(true, false, "feature/y", github.PRStateMerged, "tstapler"),
			overrideReason: "here's why",
			callerLogin:    "tstapler",
			wantAccept:     true,
		},
...
func TestReportPRCreated_should_TransitionToPRPending_When_FallbackBranchWithOverrideReason(t *testing.T) {
	...
	handler := &backlogHandlers{
		storage:              storage,
		resolveSessionBranch: func(context.Context, string) (string, error) { return "backlog/stapler-squad-ci-status-diff-viewer", nil },
		verifyPRMatchesBranch: func(context.Context, string, string, int, string) (PRVerification, error) {
			return NewPRVerification(true, false, "feature/ci-status-diff-viewer", github.PRStateMerged, "tstapler"), nil
		},
...
func TestReportPRCreated_should_RejectCall_When_UnrelatedClosedPRWithOverrideReason(t *testing.T) {
	...
	handler := &backlogHandlers{
		...
		resolveSessionBranch: func(context.Context, string) (string, error) { return "backlog/stapler-squad-ci-status-diff-viewer", nil },
		verifyPRMatchesBranch: func(context.Context, string, string, int, string) (PRVerification, error) {
			return NewPRVerification(true, false, "totally-unrelated-branch", github.PRStateClosed, "tstapler"), nil
		},
		resolveCallerGitHubLogin: func(context.Context) (string, error) { return "tstapler", nil },
	}
	ctxWithUUID := WithSessionUUID(context.Background(), sessionUUID)
...
func TestReportPRCreated_should_RejectCall_When_UnrelatedPRAuthorMismatch(t *testing.T) {
	...
	handler := &backlogHandlers{
		...
		resolveSessionBranch: func(context.Context, string) (string, error) { return "backlog/stapler-squad-ci-status-diff-viewer", nil },
		verifyPRMatchesBranch: func(context.Context, string, string, int, string) (PRVerification, error) {
			return NewPRVerification(true, false, "totally-unrelated-branch", github.PRStateOpen, "a-different-github-user"), nil
		},
		resolveCallerGitHubLogin: func(context.Context) (string, error) { return "tstapler", nil },
	}
	ctxWithUUID := WithSessionUUID(context.Background(), sessionUUID)
