...
func PortSessionHistory(ctx context.Context, oldProgram, newProgram string, i *Instance) error {
	...
	if dstAdapter.Name() == "agy" {
		...
		if err := os.MkdirAll(filepath.Dir(agyHistoryPath), 0700); err == nil {
			...
			if f, err := os.OpenFile(agyHistoryPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644); err == nil { // #nosec G304 -- agyHistoryPath is home dir + hardcoded constant components only
		...
		if err := os.MkdirAll(filepath.Dir(claudeHistoryPath), 0700); err == nil {
			...
			if hf, err := os.OpenFile(claudeHistoryPath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644); err == nil { // #nosec G304 -- claudeHistoryPath is home dir + hardcoded constant components only
