...
func TestBuildLaunchCommand_LargePromptSurvivesRealTmuxNewSession(t *testing.T) {
	...
	// can observe a torn/short read if it races the script's write, so also
	// require the read to match the expected length before accepting it.
	deadline := time.Now().Add(5 * time.Second)
	var captured []byte
	for {
		if data, readErr := os.ReadFile(outFile); readErr == nil && len(data) == len(prompt) {
			captured = data
			break
		}
