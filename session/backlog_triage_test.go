...
func TestWriteAttachedImages(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			writeAttachedImages(&sb, tc.desc, tc.dir)
			assert.Equal(t, tc.want, sb.String())
		})
	}
}

func TestHeadlessPrompts_ListAttachedImagePaths(t *testing.T) {
	...
	prompts := map[string]func(*BacklogItemData) string{
	...
	for name, build := range prompts {
		t.Run(name, func(t *testing.T) {
			assert.Contains(t, build(withImage), "- "+dir+"/a.png\n")
