...
func (_q *ShellQuery) QuerySession() *SessionQuery {
...
func (_q *ShellQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *ShellQuery) FirstIDX(ctx context.Context) string {
...
func (_q *ShellQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *ShellQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *ShellQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *ShellQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ShellQuery) WithSession(opts ...func(*SessionQuery)) *ShellQuery {
	query := (&SessionClient{config: _q.config}).Query()
...
func (_q *ShellQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Shell, error) {
	...
	if query := _q.withSession; query != nil {
		if err := _q.loadSession(ctx, query, nodes, nil,
			func(n *Shell, e *Session) { n.Edges.Session = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ShellQuery) loadSession(ctx context.Context, query *SessionQuery, nodes []*Shell, init func(*Shell), assign func(*Shell, *Session)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Shell)
	for i := range nodes {
		if nodes[i].session_shells == nil {
			continue
		}
		fk := *nodes[i].session_shells
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "session_shells" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ShellQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(shell.Table, shell.Columns, sqlgraph.NewFieldSpec(shell.FieldID, field.TypeString))
	_spec.From = _q.sql
