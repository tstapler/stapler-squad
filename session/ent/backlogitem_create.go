...
func (_c *BacklogItemCreate) AddActivityNoteIDs(ids ...uuid.UUID) *BacklogItemCreate {
	_c.mutation.AddActivityNoteIDs(ids...)
	return _c
}

// AddActivityNotes adds the "activity_notes" edges to the BacklogActivityNote entity.
func (_c *BacklogItemCreate) AddActivityNotes(v ...*BacklogActivityNote) *BacklogItemCreate {
	ids := make([]uuid.UUID, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddActivityNoteIDs(ids...)
}
...
func (_c *BacklogItemCreate) createSpec() (*BacklogItem, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.ActivityNotesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   backlogitem.ActivityNotesTable,
			Columns: []string{backlogitem.ActivityNotesColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(backlogactivitynote.FieldID, field.TypeUUID),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
