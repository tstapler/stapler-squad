// Code generated by ent, DO NOT EDIT.

package ent

import (
	"encoding/json"
	"fmt"
	"strings"
	"time"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"github.com/tstapler/stapler-squad/session/ent/approvalrule"
)

// ApprovalRule is the model entity for the ApprovalRule schema.
type ApprovalRule struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// RuleID holds the value of the "rule_id" field.
	RuleID string `json:"rule_id,omitempty"`
	// Name holds the value of the "name" field.
	Name string `json:"name,omitempty"`
	// ToolName holds the value of the "tool_name" field.
	ToolName string `json:"tool_name,omitempty"`
	// ToolPattern holds the value of the "tool_pattern" field.
	ToolPattern string `json:"tool_pattern,omitempty"`
	// ToolCategory holds the value of the "tool_category" field.
	ToolCategory string `json:"tool_category,omitempty"`
	// CommandPattern holds the value of the "command_pattern" field.
	CommandPattern string `json:"command_pattern,omitempty"`
	// FilePattern holds the value of the "file_pattern" field.
	FilePattern string `json:"file_pattern,omitempty"`
	// Decision holds the value of the "decision" field.
	Decision int `json:"decision,omitempty"`
	// RiskLevel holds the value of the "risk_level" field.
	RiskLevel int `json:"risk_level,omitempty"`
	// Reason holds the value of the "reason" field.
	Reason string `json:"reason,omitempty"`
	// Alternative holds the value of the "alternative" field.
	Alternative string `json:"alternative,omitempty"`
	// Priority holds the value of the "priority" field.
	Priority int `json:"priority,omitempty"`
	// Enabled holds the value of the "enabled" field.
	Enabled bool `json:"enabled,omitempty"`
	// Source holds the value of the "source" field.
	Source string `json:"source,omitempty"`
	// CreatedAt holds the value of the "created_at" field.
	CreatedAt time.Time `json:"created_at,omitempty"`
	// UpdatedAt holds the value of the "updated_at" field.
	UpdatedAt time.Time `json:"updated_at,omitempty"`
	// Programs holds the value of the "programs" field.
	Programs []string `json:"programs,omitempty"`
	// Subcommands holds the value of the "subcommands" field.
	Subcommands []string `json:"subcommands,omitempty"`
	// BlockedSubcommands holds the value of the "blocked_subcommands" field.
	BlockedSubcommands []string `json:"blocked_subcommands,omitempty"`
	// RequiredFlags holds the value of the "required_flags" field.
	RequiredFlags []string `json:"required_flags,omitempty"`
	// ForbiddenFlags holds the value of the "forbidden_flags" field.
	ForbiddenFlags []string `json:"forbidden_flags,omitempty"`
	// RequiredFlagPrefixes holds the value of the "required_flag_prefixes" field.
	RequiredFlagPrefixes []string `json:"required_flag_prefixes,omitempty"`
	// PythonModes holds the value of the "python_modes" field.
	PythonModes []string `json:"python_modes,omitempty"`
	// SafePythonImportsOnly holds the value of the "safe_python_imports_only" field.
	SafePythonImportsOnly bool `json:"safe_python_imports_only,omitempty"`
	selectValues          sql.SelectValues
}

// scanValues returns the types for scanning values from sql.Rows.
func (*ApprovalRule) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case approvalrule.FieldPrograms, approvalrule.FieldSubcommands, approvalrule.FieldBlockedSubcommands, approvalrule.FieldRequiredFlags, approvalrule.FieldForbiddenFlags, approvalrule.FieldRequiredFlagPrefixes, approvalrule.FieldPythonModes:
			values[i] = new([]byte)
		case approvalrule.FieldEnabled, approvalrule.FieldSafePythonImportsOnly:
			values[i] = new(sql.NullBool)
		case approvalrule.FieldID, approvalrule.FieldDecision, approvalrule.FieldRiskLevel, approvalrule.FieldPriority:
			values[i] = new(sql.NullInt64)
		case approvalrule.FieldRuleID, approvalrule.FieldName, approvalrule.FieldToolName, approvalrule.FieldToolPattern, approvalrule.FieldToolCategory, approvalrule.FieldCommandPattern, approvalrule.FieldFilePattern, approvalrule.FieldReason, approvalrule.FieldAlternative, approvalrule.FieldSource:
			values[i] = new(sql.NullString)
		case approvalrule.FieldCreatedAt, approvalrule.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		default:
			values[i] = new(sql.UnknownType)
		}
	}
	return values, nil
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the ApprovalRule fields.
func (_m *ApprovalRule) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case approvalrule.FieldID:
			value, ok := values[i].(*sql.NullInt64)
			if !ok {
				return fmt.Errorf("unexpected type %T for field id", value)
			}
			_m.ID = int(value.Int64)
		case approvalrule.FieldRuleID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field rule_id", values[i])
			} else if value.Valid {
				_m.RuleID = value.String
			}
		case approvalrule.FieldName:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field name", values[i])
			} else if value.Valid {
				_m.Name = value.String
			}
		case approvalrule.FieldToolName:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tool_name", values[i])
			} else if value.Valid {
				_m.ToolName = value.String
			}
		case approvalrule.FieldToolPattern:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tool_pattern", values[i])
			} else if value.Valid {
				_m.ToolPattern = value.String
			}
		case approvalrule.FieldToolCategory:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tool_category", values[i])
			} else if value.Valid {
				_m.ToolCategory = value.String
			}
		case approvalrule.FieldCommandPattern:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field command_pattern", values[i])
			} else if value.Valid {
				_m.CommandPattern = value.String
			}
		case approvalrule.FieldFilePattern:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field file_pattern", values[i])
			} else if value.Valid {
				_m.FilePattern = value.String
			}
		case approvalrule.FieldDecision:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field decision", values[i])
			} else if value.Valid {
				_m.Decision = int(value.Int64)
			}
		case approvalrule.FieldRiskLevel:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field risk_level", values[i])
			} else if value.Valid {
				_m.RiskLevel = int(value.Int64)
			}
		case approvalrule.FieldReason:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field reason", values[i])
			} else if value.Valid {
				_m.Reason = value.String
			}
		case approvalrule.FieldAlternative:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field alternative", values[i])
			} else if value.Valid {
				_m.Alternative = value.String
			}
		case approvalrule.FieldPriority:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field priority", values[i])
			} else if value.Valid {
				_m.Priority = int(value.Int64)
			}
		case approvalrule.FieldEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field enabled", values[i])
			} else if value.Valid {
				_m.Enabled = value.Bool
			}
		case approvalrule.FieldSource:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field source", values[i])
			} else if value.Valid {
				_m.Source = value.String
			}
		case approvalrule.FieldCreatedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field created_at", values[i])
			} else if value.Valid {
				_m.CreatedAt = value.Time
			}
		case approvalrule.FieldUpdatedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field updated_at", values[i])
			} else if value.Valid {
				_m.UpdatedAt = value.Time
			}
		case approvalrule.FieldPrograms:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field programs", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.Programs); err != nil {
					return fmt.Errorf("unmarshal field programs: %w", err)
				}
			}
		case approvalrule.FieldSubcommands:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field subcommands", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.Subcommands); err != nil {
					return fmt.Errorf("unmarshal field subcommands: %w", err)
				}
			}
		case approvalrule.FieldBlockedSubcommands:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field blocked_subcommands", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.BlockedSubcommands); err != nil {
					return fmt.Errorf("unmarshal field blocked_subcommands: %w", err)
				}
			}
		case approvalrule.FieldRequiredFlags:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field required_flags", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.RequiredFlags); err != nil {
					return fmt.Errorf("unmarshal field required_flags: %w", err)
				}
			}
		case approvalrule.FieldForbiddenFlags:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field forbidden_flags", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ForbiddenFlags); err != nil {
					return fmt.Errorf("unmarshal field forbidden_flags: %w", err)
				}
			}
		case approvalrule.FieldRequiredFlagPrefixes:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field required_flag_prefixes", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.RequiredFlagPrefixes); err != nil {
					return fmt.Errorf("unmarshal field required_flag_prefixes: %w", err)
				}
			}
		case approvalrule.FieldPythonModes:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field python_modes", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.PythonModes); err != nil {
					return fmt.Errorf("unmarshal field python_modes: %w", err)
				}
			}
		case approvalrule.FieldSafePythonImportsOnly:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field safe_python_imports_only", values[i])
			} else if value.Valid {
				_m.SafePythonImportsOnly = value.Bool
			}
		default:
			_m.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the ApprovalRule.
// This includes values selected through modifiers, order, etc.
func (_m *ApprovalRule) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// Update returns a builder for updating this ApprovalRule.
// Note that you need to call ApprovalRule.Unwrap() before calling this method if this ApprovalRule
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *ApprovalRule) Update() *ApprovalRuleUpdateOne {
	return NewApprovalRuleClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the ApprovalRule entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *ApprovalRule) Unwrap() *ApprovalRule {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: ApprovalRule is not a transactional entity")
	}
	_m.config.driver = _tx.drv
	return _m
}

// String implements the fmt.Stringer.
func (_m *ApprovalRule) String() string {
	var builder strings.Builder
	builder.WriteString("ApprovalRule(")
	builder.WriteString(fmt.Sprintf("id=%v, ", _m.ID))
	builder.WriteString("rule_id=")
	builder.WriteString(_m.RuleID)
	builder.WriteString(", ")
	builder.WriteString("name=")
	builder.WriteString(_m.Name)
	builder.WriteString(", ")
	builder.WriteString("tool_name=")
	builder.WriteString(_m.ToolName)
	builder.WriteString(", ")
	builder.WriteString("tool_pattern=")
	builder.WriteString(_m.ToolPattern)
	builder.WriteString(", ")
	builder.WriteString("tool_category=")
	builder.WriteString(_m.ToolCategory)
	builder.WriteString(", ")
	builder.WriteString("command_pattern=")
	builder.WriteString(_m.CommandPattern)
	builder.WriteString(", ")
	builder.WriteString("file_pattern=")
	builder.WriteString(_m.FilePattern)
	builder.WriteString(", ")
	builder.WriteString("decision=")
	builder.WriteString(fmt.Sprintf("%v", _m.Decision))
	builder.WriteString(", ")
	builder.WriteString("risk_level=")
	builder.WriteString(fmt.Sprintf("%v", _m.RiskLevel))
	builder.WriteString(", ")
	builder.WriteString("reason=")
	builder.WriteString(_m.Reason)
	builder.WriteString(", ")
	builder.WriteString("alternative=")
	builder.WriteString(_m.Alternative)
	builder.WriteString(", ")
	builder.WriteString("priority=")
	builder.WriteString(fmt.Sprintf("%v", _m.Priority))
	builder.WriteString(", ")
	builder.WriteString("enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.Enabled))
	builder.WriteString(", ")
	builder.WriteString("source=")
	builder.WriteString(_m.Source)
	builder.WriteString(", ")
	builder.WriteString("created_at=")
	builder.WriteString(_m.CreatedAt.Format(time.ANSIC))
	builder.WriteString(", ")
	builder.WriteString("updated_at=")
	builder.WriteString(_m.UpdatedAt.Format(time.ANSIC))
	builder.WriteString(", ")
	builder.WriteString("programs=")
	builder.WriteString(fmt.Sprintf("%v", _m.Programs))
	builder.WriteString(", ")
	builder.WriteString("subcommands=")
	builder.WriteString(fmt.Sprintf("%v", _m.Subcommands))
	builder.WriteString(", ")
	builder.WriteString("blocked_subcommands=")
	builder.WriteString(fmt.Sprintf("%v", _m.BlockedSubcommands))
	builder.WriteString(", ")
	builder.WriteString("required_flags=")
	builder.WriteString(fmt.Sprintf("%v", _m.RequiredFlags))
	builder.WriteString(", ")
	builder.WriteString("forbidden_flags=")
	builder.WriteString(fmt.Sprintf("%v", _m.ForbiddenFlags))
	builder.WriteString(", ")
	builder.WriteString("required_flag_prefixes=")
	builder.WriteString(fmt.Sprintf("%v", _m.RequiredFlagPrefixes))
	builder.WriteString(", ")
	builder.WriteString("python_modes=")
	builder.WriteString(fmt.Sprintf("%v", _m.PythonModes))
	builder.WriteString(", ")
	builder.WriteString("safe_python_imports_only=")
	builder.WriteString(fmt.Sprintf("%v", _m.SafePythonImportsOnly))
	builder.WriteByte(')')
	return builder.String()
}

// ApprovalRules is a parsable slice of ApprovalRule.
type ApprovalRules []*ApprovalRule
