...
func buildDefaultSlashCommandSet(item *BacklogItemData) (map[string]string, error) {
	...
	files["ship.md"] = fmt.Sprintf("You are ready to ship your work as a pull request — either because /backlog/review just "+
		...
		"   Call the report_pr_created MCP tool with item_id=%s, pr_url=<the PR URL /github:pr-ship or gh pr "+
		"create printed>, pr_number=<the PR number>, and summary=<2-3 sentences: what changed and why>.\n"+
		"   You created this PR yourself — nothing else will ever report it back to the item record. Skipping "+
		"this step leaves the item stuck in review with a real PR that is invisible to the reviewer and the "+
		"operator.\n\n"+
...
func addWorktreeExcludes(worktreePath string) {
	...
	f, openErr := os.OpenFile(excludeFile, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600)
	if openErr != nil {
	...
	for _, p := range git.ScaffoldingExcludePatterns {
...
// git.ScaffoldingExcludePatterns and the PR that added this function for the full incident
// history). GitWorktree.StageAllExceptScaffolding (session/git/worktree_git.go) is the
// complementary guard that closes the same gap at commit time, for scaffolding that gets
// tracked mid-session rather than inherited from a stale branch.
