...
func TestExtractFromToolResult_ExternalURLCap(t *testing.T) {
	// Generate 60 unique URLs — should be capped at maxExternalURLs (50).
	...
	// cap50 is applied at merge time, not here — so all 60 may be returned.
	// Test that dedup works: all are unique, all 60 should appear.
	if len(urls) != 60 {
