...
// does not also set Setpgid. On Darwin, setsid(2) makes the caller both a
...
func TestBuildSysProcAttr_Setsid_excludes_Setpgid(t *testing.T) {
	...
	if attr.Setpgid {
		t.Error("Setpgid must be false when Setsid is true: setting both causes EPERM on Darwin")
	}
}
