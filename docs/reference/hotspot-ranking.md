...
| 5 | `server/mcp/tools_backlog.go` | 514 | 51 | ~26k | **done** — `reportPRCreated` (gocyclo 51, the file's highest-complexity function; report_pr_created's whole handler) and its exclusive helpers (`sessionBranch`, `verifyPR`, `callerGitHubLogin`, `decideOverridePolicy`, plus the `reportPRCreatedAllowedSourceStatuses` map) extracted to `tools_backlog_pr.go`, pure file move — this file already used `// --- tool_name ---` section-comment boundaries internally, so the split followed an existing seam rather than inventing one. `findSessionTitleByUUID`, positioned adjacently, stayed behind — it's `submitReviewVerdict`'s helper, not this cluster's. `docs/reference/hotspot-ranking.md`'s note about rows 2-3 outranking row 4 no longer applies; all of rows 2-5 are now done. Commit [`2712a5f92`](https://github.com/tstapler/stapler-squad/commit/2712a5f92) (gocognit/gocyclo/funlen new-code-gate suppression on the relocated `reportPRCreated`, same shape as row 1's). Second-highest function in the file, `submitTriageResult` (gocyclo 49) plus its duplicate/triage-detection cluster (`reportDuplicate`, `resolveDuplicateRef`, `reportDuplicateUnclaimed`, `verifyRef`, `verifyGitHubRefExists`), is a natural follow-up split (also a contiguous, existing section) — not required before moving to row 6. |
