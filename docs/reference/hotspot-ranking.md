...
| 2 | `session/tmux/tmux.go` | 439 | 149 | ~65k | pending |
| 3 | `server/services/connectrpc_websocket.go` | 507 | 107 | ~54k | pending |
| 4 | `server/services/backlog_service_triage.go` | 566 | 94 | ~53k | **in-progress** — diagnosed 2026-09-12: `TriggerTriage` (517 lines) mirrors `StreamTerminal`'s shape (sync RPC-handler setup wrapping one large async goroutine body); `TriggerReReview` (460 lines) is a long linear synchronous handler. Plan: extract both (+ exclusive helpers) into their own files, same pattern as row 1. |
