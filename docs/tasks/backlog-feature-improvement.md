...
| `61371a09` | (separate from the above) `BOUNCING` verdict `PARTIAL` | Real regression in the item's own diff, not a pipeline bug: `recoverOrUpdateOwner` (`gandalf.go:671-673`) now hard-errors when `removeIDGroupMembers` fails on an already-existing group, where the pre-existing `UpdatePolicyOwner` only `slog.Warn`'d and continued — verified against `git show 3e62d67^`. | App-code bug in the item's own AC2, route as a normal fix within that item's own work, unrelated to backlog plumbing |
